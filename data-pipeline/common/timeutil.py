...
# KRX 휴장일(평일만). 출처는 KRX 가 공표하는 휴장일 표다(open.krx.co.kr '휴장일' 조회,
# gridTp=KRX). 2026-09-23 에 2025~2027년치를 받아 그대로 옮겼다.
#
# ⚠️ **해마다 다음 해를 채울 것.** 표에 없는 해는 평일을 전부 개장일로 센다 — 지금까지의
#    동작 그대로라 조용히 틀리지는 않고, 대신 설·추석 뒤에 check_freshness 가 다시 헛알림을
#    낸다. 그 스크립트가 표에 올해가 없으면 경고를 찍는다.
# ⚠️ 수집 쪽 business_days 에는 쓰지 않는다. 표가 틀려 연 날을 휴장으로 적으면 그날 자료를
#    영영 안 받게 된다. 여기는 '며칠 늦었나'를 세는 데만 쓴다.
...
KRX_HOLIDAY_YEARS = frozenset({2025, 2026, 2027})


def krx_trading_days(start: date, end: date):
    """start~end(양끝 포함) 사이의 KRX 개장일. business_days 에서 KRX_HOLIDAYS 를 뺀 것."""
    for d in business_days(start, end):
        if d.isoformat() not in KRX_HOLIDAYS:
            yield d
