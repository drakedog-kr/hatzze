...
NIGHT_FROM_HOUR = 18
# 밤과 낮의 경계. 아침 글(morning2)은 '어제 18시 ~ 오늘 이 시각', 저녁 글(evening2)은 '오늘 이
# 시각 ~ 18시'를 요약해서, 두 글이 이 시각에서 맞물린다.
# ⚠️ 아침 수집이 끝나는 시각보다 늦으면 안 된다. 그 사이 올라온 글은 아침 글엔 아직 없고
#    저녁 글 구간 밖이라 어느 글에도 안 들어간다. 2026-09-23 발사를 06:30 으로 당기면서
#    수집 끝이 06:56~07:00 이 되어 8 → 7 로 내렸다(8 이면 그 틈이 60분이 된다).
NIGHT_TO_HOUR = 7
...
def night_split(db, target: date) -> tuple[int, int]:
    ...
    '밤'은 18시 이후 또는 7시 이전이다. 장 마감(15:30) 한참 뒤부터 이튿날 장 준비 전까지.
...
def load_night_messages(db) -> tuple[int, list[str], datetime, datetime]:
    """**지난밤**(어제 18시 ~ 오늘 7시, KST) 메시지 수와 널리 퍼진 순 발췌.
...
# ⚠️ [표현]은 Opus 5.5 로 옮기며 다시 썼다(2026-09-23). 옛 규칙은 "텔레그램에서 오간 말이니
#    화제·전언으로 적으라" 한 줄이었다. Haiku 는 그걸 반쯤 무시해 왔고 지금까지 나간 글의 말투가
#    그 결과다. Opus 는 문장마다 따라 거의 모든 문장을 "~다는 소식이 여러 채널에 퍼졌습니다"로
#    감쌌다 — 요약이 아니라 남의 말을 옮기는 글이 된다. 그래서 뉴스·공시는 바로 쓰고, 언론
#    보도는 매체를 주어로 세우고, 전언은 확인되지 않은 말(전망·해석·소문)에만 붙이게 나눴다.
#    채널을 거쳐 전하는 문장('돌았/퍼졌/전해졌/회자/오갔/화제였/채널에서/거론·언급됐' ·
#    '~다는 소식·이야기·말·글·보도')을 한 편에 몇 개 쓰나, 다섯 형식 같은 재료 4회씩:
#      프로덕션 Haiku 3주 1.5 · 같은 재료 Haiku 1.8 · 새 규칙 Opus 2.0
#      (옛 규칙 Opus 는 그날 오후 재료 3회로 7.2)
#    토요일 글만 4~5 로 높다. 그 주 재료에 ETF 편입설·계약설처럼 확인 안 된 설이 많았다.
#    주가 표현(목표가·매수세·급등 등)은 같은 재료 Haiku 20편에 여럿("목표가를 49만 원으로
#    제시") · Opus 0.
# ⚠️ [증권사 의견]은 2026-09-24 에 풀었다. 목표주가는 '올렸다·내렸다' 소식까지 숫자 없이,
#    실적·제품 가격·업황 전망은 누가 봤는지를 붙여 전한다. 옛 규칙은 목표가와 '오를 것'을
#    통째로 막아 Opus 가 메모리 가격 전망까지 한 줄도 안 썼다. 주가 예측·주가 평가는 계속 막는다.
#    바꾼 뒤 다섯 형식 4회씩: 목표주가 문장은 전부 숫자 없이 · 주가 예측 0 · 글 길이는 같은 재료
#    Haiku 보다 짧거나 같다(마감 631 → 562자, 전망이 다시 실려 478 에서 늘었다).
_LLM_RULES = """\
...
[절대 하지 말 것]
- 매수/매도/투자 권유, 주가가 오를지 내릴지 예측('주가가 오를 것', '앞으로 오른다').
...
[⚠️ 증권사 의견은 이만큼만 옮기세요]
- **주가를 두고 한 평가는 옮기지 마세요.** "지금 주가는 과도한 조정", "저평가 국면", "매수 의견",
  "비중확대", "상승 여력" 같은 말은 우리가 전하는 순간 매수·매도 권유로 읽힙니다.
  (X) "씨티는 삼성전자 주가 하락이 과도하다고 지적했습니다."
- **목표주가는 올렸다·내렸다는 소식까지만, 숫자 없이** 씁니다.
  (O) "○○증권이 △△의 목표주가를 올렸습니다." (X) "목표주가를 □□원으로 제시했습니다."
- **실적·제품 가격·업황 전망은 누가 그렇게 봤는지를 붙여** 씁니다.
  (O) "○○는 △△ 가격이 더 오를 것으로 봤습니다."
...
[표현]
- 이 글은 여러 채널의 이야기를 추려 **무슨 일이 있었는지** 알려 주는 요약입니다. 채널을
  요약한 글이라는 건 독자가 이미 압니다. 문장마다 "~다는 소식이 돌았습니다", "여러 채널에서
  ~", "~가 회자됐습니다"로 감싸면 남의 말을 그대로 옮기는 글이 됩니다.
...
- **전언으로 적는 건 확인되지 않은 말뿐입니다.** 채널의 전망·해석·풀이, 출처 없는 소문이
  ...
  "~를 체결했습니다"처럼 단정하지 마세요.
- **언론 보도는 매체를 주어로 세워 씁니다.** 예: "○○ 매체는 △△ 주문이 늘었다고 보도했습니다."
  "~라는 보도가 전해졌습니다", "~라는 글이 퍼졌습니다"처럼 채널을 거쳐 감싸지 마세요.
- 그래서 "돌았습니다", "퍼졌습니다", "전해졌습니다", "회자됐습니다", "오갔습니다" 같은 말은
  대개 한 갈래에 한 번이면 됩니다."""
...
# 언급 점유율", "증권사의 주가 평가·목표주가 숫자 금지", "주가 예측 금지"를 들고 있다.
BANNED_TERMS = (
    "매매 신호",
    "매수 의견", "매도 의견",
    "매수 추천", "매도 추천",
...
#   (없어짐) 증권계좌에 대기 중인 매수 자금 ⊃ 매수  investor_deposit → limit_up_breadth
#            2026-07-31 에 '급등 종목 강도'로 갈렸다. 새 이름·헤드라인에 금지 조합이 없다.
