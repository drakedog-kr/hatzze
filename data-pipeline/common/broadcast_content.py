...
#
# 앞의 셋은 채널에 나갈 문장을 막는다. 넷째만 방향이 반대다 — 모델에게 시황 어휘를 쥐여
# 주지 않으려고 **입력**을 거른다(safe_brief 주석). 그 자리를 안 막으면 첫째 그물이 대신
# 걸려서, 모델이 옮겨 적은 값으로 문단이 통째로 사라진다.
#
# 오래도록 첫째만 있었다. 둘째·셋째는 표에 저장된 문장을 그대로 읽어 오는 자리라
