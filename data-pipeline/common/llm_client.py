...
# ── Opus 를 부를 때만 달라지는 것 ──────────────────────────────────────────────
# 2026-09-23 부터 사람이 가장 먼저 읽는 글 두 자리(카더라 총평 대목 · 채널 발송 글)만
# Opus 5.5 로 쓴다. 나머지는 Haiku 그대로다(분류·감성은 과열지수 눈금이 Haiku 출력에 맞춰져
# 있어 바꾸면 안 된다). 히어로 요약도 견줘 봤지만 말투가 같고 길이만 늘어 Haiku 로 뒀다.
#
