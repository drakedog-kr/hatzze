...
1판 포맷은 그대로 두었다(send_telegram_broadcast.py 의 morning·evening·theme·weekly).
이 판이 채택되면 워크플로의 `--format` 만 바꾸면 된다. 그때 같이 손볼 것 둘:
  · A 는 08:30 전에 도착해야 한다 — 발송 스텝을 KRX 게이트 앞으로(daily-update.yml).
  · 토요일 글이 쓰는 미장 까닭(telegram_us_stock_move_reason)은 지금 저녁 실행에서만
    만들어진다(generate_move_reasons.MIN_DAY_MSGS). 토요일 아침에도 미장 몫은 만들게 해야
    금요일 세션 까닭이 14:00 글에 실린다. 안 만들면 그 줄만 빠지고 갈래는 그대로 나간다.
