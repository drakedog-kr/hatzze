...
INDICATOR_THRESHOLDS = {
    ...
    # ※ 6%가 관측 최대라 천장 포화가 생긴다. 표본이 20일뿐이라 잠정 — 위쪽 값이 더 쌓이면
    #   ceiling 을 다시 볼 것(옛 주석의 "2·4·5% 세 종류뿐"은 그 사이 6%가 나와 이미 낡았다).
    "bestseller_finance_ratio": {"kind": "fixed", "threshold": 6.0, "floor": 2.0},
    ...
    # foreign_sell_at_high: 지수가 52주 고점 −5% 이내일 때의 외국인 순매도, 최근 5거래일
    # 누적(억원). 고점권이 아니거나 외국인이 순매수면 0 이다(스크립트가 이미 게이트를
    # 적용해 저장하므로 여기 값은 언제나 0 이상 — floor 를 안 준다).
    #
    # 2026-07-31 신설. 자리와 가중치(2.5)는 individual_net_buy 에서 물려받았고, 그 지표는
    # 소스(네이버 금융 스크래핑)가 robots.txt 로 막혀 있어 내렸다. 왜 주체를 개인에서
    # 외국인으로 바꾸면서 게이트가 필요했는지는 fetch_foreign_sell_at_high.py 머리말 참고.
    #
    # 진입선 1조는 **창을 어떻게 잡아도 같은 값**이 나왔다(ECOS 실측):
    ...
    #
    # ⚠️ **값이 0 인 날이 86%(최근 5년)** 인데 이건 정보 손실이 아니다. market_actions 의
    # 옛 공식이 86% 를 0 으로 뭉갰던 것과는 다르다 — 그쪽은 실제 이벤트가 있던 날까지
    # 버렸고, 이쪽의 0 은 "고점권이 아니거나 외국인이 사는 중" 이라는 사실 자체다.
    # 연속 계기가 아니라 조건이 켜졌을 때만 울리는 지표로 읽어야 한다.
    "foreign_sell_at_high": {"kind": "fixed", "threshold": 10_000.0},
    # limit_up_breadth: 장중 급등 종목 수를 가중 합산한 점수(3×상한가 + 2×(+20~29%) + 1×(+10~20%)).
    # 자리와 카드는 investor_deposit(고객예탁금)에서 물려받았다 — 그쪽 소스가 네이버 금융
    # 스크래핑이었고 finance.naver.com 의 robots.txt 가 전면 금지라 내렸다.
    #
    # 기준선을 고정값이 아니라 **최근 1년 평균**으로 두는 이유: 상장 종목이 2021년
    # 2,300개대에서 2,700개대로 늘어, 개수를 고정 임계값에 걸면 그 증가만으로 과열도가
    # 올라간다. 기준선이 같이 움직이면 그 문제가 사라진다(예탁금이 겪던 것과 같은 함정).
    #
    # ±75%: 평균이면 상온(50) · 평균 대비 +75%면 초고온(100). 코스피만 1,341일 실측으로
    # 천장 포화 9.2%(put_call 5% 언저리) · 바닥 포화 0.4% 다.
    # ±50%는 천장이 16.3%로 헐겁고, ±110%는 바닥이 0%가 되어 아래쪽을 통째로 잃는다.
    #
    # ⚠️ 코스닥을 뺀 뒤 다시 잡은 값이다. 둘을 합쳐 재던 때는 ±50% 였는데, 종목이 1/3로
    # 줄자 하루 값의 상대 변동이 커져(1년 평균 대비 p90 +45.8% → +70.7%) 같은 밴드면
    # 천장에 두 배로 붙었다. 세는 시장을 바꾸면 눈금도 반드시 다시 잡아야 한다.
    #
    # ⚠️ **이 지표는 과열보다 변동성에 가깝다.** 국면별 실측 중앙(코스피만, ±75% 기준):
    # 하락장 43.7 · 평범 42.4 · 강세 43.4 · 회복 51.7 · **2026 폭락 78.5**. 급등 종목은 시장이 어느 쪽으로든 크게
    # 흔들릴 때 함께 늘기 때문이다. 상승−하락 상쇄·고점권 게이트·상한가 유지율 등 10형태를
    # 1,366거래일로 시험했지만 광풍과 폭락을 가르는 조합이 없었다(46,225개 전수 탐색에서
    # 21%가 '분리'에 성공 — 그 통과율 자체가 신호가 아니라는 증거).
    # 그래서 **가중치를 1.0** 으로 낮춰 들여놓는다(예탁금은 3.0 이었다). 값이 틀려도 종합
    # 점수는 1점 안쪽으로 움직인다. 판별력이 확인되면 그때 올린다.
    "limit_up_breadth": {"kind": "rolling_average", "window": 252, "surge_map": {"floor": -75.0, "ceil": 75.0}},
