...
INDICATOR_THRESHOLDS = {
    ...
    # limit_up_breadth: 코스피에서 장중 10% 넘게 오른 종목의 비율(%).
    ...
    # 2,300개대 → 2,700개대) 임계값이 금방 낡는데(예탁금이 겪던 함정), 분모가 전체 종목
    # 수라 그 증가가 상쇄된다. 앞선 안(가중 합산)은 개수 기반이라 롤링 평균 기준선이
    # 필요했지만 그 장치가 통째로 사라졌다.
    #
    # ceiling 5%: 코스피만 1,341거래일(2021-02~2026-07) 실측으로 전체 도달률 3.7%,
    # 평범한 해는 0.4~2.0% 만 닿는다. 4%는 전체 8.0%로 헐겁고, 6%는 평범한 해가 0.0%라
    # 아예 안 닿는다. floor 는 0 — 실측 최소가 0.52% 라 바닥에 붙는 날이 없다.
    #
    # ⚠️ **이 지표는 과열보다 변동성에 가깝다.** 국면별 실측 중앙: 하락장 1.91% · 회복
    # 2.00% · 평범 1.89% · 강세 1.87% · 2021(광풍 포함) 2.17% · **2026 폭락 3.60%**.
    # 다섯 국면이 1.9~2.2% 로 붙어 있고 폭락만 튄다. 급등 종목은 시장이 어느 쪽으로든
    # 크게 흔들릴 때 함께 늘기 때문이다. 상승−하락 상쇄·고점권 게이트·상한가 유지율 등
    # 10형태를 1,366거래일로 시험했고 가중치 46,225개 조합을 전수 탐색했지만 광풍과
    # 폭락을 가르는 조합이 없었다(그 탐색에서 21%가 '분리'에 성공 — 통과율 자체가 신호가
    # 아니라는 증거다).
    # 그래서 **가중치를 1.0** 으로 낮춰 들여놓는다(예탁금은 3.0 이었다). 값이 틀려도 종합
    # 점수는 1점 안쪽으로 움직인다. 판별력이 확인되면 그때 올린다.
    "limit_up_breadth": {"kind": "fixed", "threshold": 5.0},
