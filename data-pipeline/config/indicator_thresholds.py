...
INDICATOR_THRESHOLDS = {
    # 버핏지수: floor-ceiling — 시총/GDP 105%=0%(정상), 265%=100%. 재보정 배경:
    # 1년 분포상 100%는 사실상 바닥(p10)이라 늘 최대치로 고정됐고, 235%(p90, 상위10%)
    # 이상에서만 "과열→하락" 신호가 나왔다. ceiling은 기여 상한이지 표시값 상한이 아니다.
    #
    # 2026-07-29: 초고온 진입선을 202.5 → **225**로 올린다(ceiling 235 → 265, floor 유지).
    # 옛 ceiling 235 는 실측 1년의 **9.0%가 천장 포화**라, 그 날들이 전부 100으로 같게 찍혀
    # 고점끼리를 구분하지 못했다. 265 로 열면 포화가 0.4%로 떨어지고 초고온도 18.8% → 12.5%
    # 로 다른 지표들(10~16%)과 엄격도가 맞는다. 고점창−저점창 스프레드는 10.69 → **12.55**
    # 로 오히려 좋아진다 — 포화됐던 9%가 대부분 고점 부근 날이었기 때문이다.
    # ※ floor 105 는 유지한다. 바닥 포화 24.6%는 floor 가 정하는 값이라 ceiling 을 올려도
    #   안 변한다. 그건 별개 건이라 여기서 같이 손대지 않는다.
    "buffett_index": {"kind": "fixed", "threshold": 265.0, "floor": 105.0},
    # kospi_high_gap: ATH 근접도. 피스와이즈 — kink(−5%)~ceiling(+5%) = 75~100%,
    # floor(−35%)~kink = 0~75%.
    ...
    # 0.4%로 떨어진다. 그때 kink 도 −3 → −1.5 로 올려 초고온 비율을 60% → 46%로 낮췄다.
    # floor=−35%는 유지 — 코스피 10년 낙폭 실측(1%ile −33%·5%ile −28%) 근거이고, 이번
    # 표본(최대 −28.5%)만 보고 좁히면 진짜 약세장에서 바로 포화된다.
    #
    # 2026-07-29: kink 를 −1.5 → **−5.0** 으로 내린다. "전고점 −5% 안쪽이면 고점 부근"으로
    # 보겠다는 판단이라 초고온 진입선이 그 지점에 놓인다. 07-23 과 **반대 방향**이라 비율은
    # 다시 올라간다 — backtest/current.py 로 재보면 이 지표의 중앙 과열도가 74.1 → 82.7,
    # 초고온이 **41.4% → 60.6%** 다(07-23 에 "너무 헐겁다"고 판단해 낮췄던 60%로 되돌아온
    # 셈이다). 가중치 2위 지표가 날의 6할을 초고온으로 보내니 카드 배지도 그만큼 자주 켜진다.
    # 다만 이 표본은 코스피가 세 배가 된 해라 전고점 근처에 머문 날이 유난히 많았다.
    # 종합점수 쪽 영향은 작다 — 같은 날 내린 bestseller 조정과 거의 상쇄돼 중앙 54 유지,
    # 국면 비중 상온 40→39% · 고온 44→45% · 초고온 16% 동일, 고점 판독 78/87/85 그대로다.
    # SCORE_DISPLAY_ANCHORS 는 손대지 않아도 된다.
    "kospi_high_gap": {"kind": "fixed", "threshold": 5.0, "floor": -35.0, "kink": -5.0},
    ...
    # pooled_sentiment_details). raw_value 가 이제 하루치가 아니라 최근 3일 합산 비율이다.
    #
    # 2026-08-06 디시 재보정: floor −7.5 / ceiling +2.5 → **floor −30 / ceiling +10**.
    #
    # ■ 증상 — 07-24 이후 풀링값 15일이 **전부** floor 밖이라 과열도가 100% 0이었다.
    #   가중치 2.0(43.0 중 4.7%)짜리 공개 지표가 13일째 죽어 있었다.
    #
    # ■ 원인은 풀링이 아니다. 07-23 눈금을 07-24 풀링이 망가뜨렸다고 읽기 쉬운데, 일별
    #   원자(details.pos_count_1d…)로 되돌려 보면 **풀링 없이도 LLM 시절 17일 중 12일
    #   (71%)이 이미 바닥**이었다. 진짜 원인은 07-23 눈금이 **키워드 분류 시절 분포 위에
    #   얹혔다**는 것이다 — 그때 있던 행 15개 중 11개가 키워드 시절(중앙 −1.24, 범위
    #   −4.6~+0.15)이고 LLM 행은 나흘뿐이었다. LLM 전환으로 분포가 통째로 12.2pt 내려앉아
    #   (일별 중앙 −1.24 → −13.40) 눈금 전체가 데이터 위쪽에 떠 버렸다. 풀링은 일별 잡음으로
    #   가끔 floor 위를 찌르던 5일마저 없앤 것뿐이다.
    #   ※ 같은 날 같이 풀링을 켠 news_sentiment 가 멀쩡한 이유도 여기 있다. 뉴스는 키워드
    #     시절에도 −26.6~+41.5 로 넓게 흔들려 눈금이 넓게(−50/+65) 잡혔고, 그 폭이 LLM 시절
    #     분포(−48~+35)를 그대로 덮었다. 디시만 키워드 시절 범위가 10pt 도 안 됐다.
    #     뉴스는 지금 28행 중 바닥 0·천장 0·중앙 41.5(범위 1~87)라 **손대지 않는다.**
    #
    # ■ 새 눈금은 분위수가 아니라 **의미**로 잡았다. 표본 15일이 전부 코스피 전고점 −22~−39%
    #   구간이라 분위수로 맞추면 시장이 정상으로 돌아왔을 때 천장에 붙는다(아래 kospi_speed_60d
    #   주석의 함정). raw 를 '분류된 글 중 긍정 비율'로 되읽어(raw = (2p−1)×c×100, 비중립
    #   비중 c≈0.52) 세 지점을 손으로 놓았다:
    #     · 과열도 100 = raw **+10** — 긍정 글이 부정 글의 1.5배. 주갤에선 진짜 광기다.
    #     · 과열도  75 = raw **0** — 긍정과 부정이 같아지는 지점. 구조적으로 부정이 이기는
    #       갤러리라 이 균형점 자체가 froth 신호다(초고온 진입선이 여기 놓인다).
    #     · 과열도   0 = raw **−30** — 부정이 긍정의 3.7배. 관측 최악(일별 −35.5)보다는 위,
    #       풀링 최악(−23.3)보다는 6.7pt 아래라 이번 폭락이 바닥에 눌어붙지 않는다.
    #   교차검증: 풀링값을 전고점 괴리에 회귀하면 dc = 2.97 + 0.6515×gap (r=+0.705, n=15).
    #   이 기울기로 보면 코스피가 전고점일 때 raw ≈ +3.0 → 과열도 82(초고온), 전고점 −5%면
    #   74, 평범한 −15%면 58, 지금(−28%)이 36 이다. froth 게이지가 가리켜야 할 방향과 맞는다.
    #
    # ■ 남은 한계 — 비폭락장 표본이 **하루도 없다**. 위 회귀는 관측 구간(−22~−39%) 밖으로
    #   17~34pt 외삽한 것이라 '평범한 장'의 절대 수준은 아직 추정이다. 코스피가 전고점
    #   −15% 안쪽으로 올라온 날이 30일쯤 쌓이면 그 구간 실측으로 floor 를 다시 볼 것.
    #   그때까지 이 눈금이 틀렸다면 방향은 '너무 뜨겁게 읽힌다' 쪽이다(초고온이 잦아진다).
    "dcinside_post_count": {"kind": "fixed", "threshold": 10.0, "floor": -30.0},
    ...
    # 초고온 진입은 급증 +50.0%(= floor + 0.75×(ceil−floor)).
    ...
    #
    # 2026-07-29: 초고온 진입선을 3.75% → **5%**로 올린다. floor 없이 threshold 만 6.67로
    # 올려도 진입선은 5%가 되지만, 그러면 관측 최저 2%가 과열도 30에 붙어 이 파일이
    # 다른 지표에서 계속 걷어낸 **구조적 하한**이 생긴다. 그래서 floor-ceiling 으로 바꾼다 —
    # floor 2 / ceiling 6 이면 2 + 0.75×4 = 5.0 이라 진입선이 정확히 5%에 놓이고 하한도 0이다.
    # 20일 실측(2%×10 · 4%×3 · 5%×4 · 6%×3)에서 2→0 · 4→50 · 5→75 · 6→100 이 된다.
    # 초고온에서 빠지는 건 전부 4%다 — 옛 눈금에선 4%가 과열도 80이라 이미 초고온이었는데,
    # 100위 중 4권은 상온으로 보는 게 맞다. backtest/current.py 기준 중앙 과열도 60.0 → 25.0,
    # **구조적 하한 40.0 → 0**(종합 원점수의 상시 하한도 9.2 → 8.5점으로 내려간다).
    #
    # 2026-08-01: 진입선을 5% → **6%**로 한 칸 더 올린다(ceiling 6.0 → 7.33). 바로 위 문단이
    # 예고한 "천장 포화"가 실제로 문제가 됐다 — 23일 실측에서 **초고온이 39.1%**로 지표 중
    # 가장 헐거웠고 13.0%가 천장에 붙어 있었다. 6%(=지금까지 본 가장 뜨거운 날)를 진입선으로
    # 놓으면 초고온 39.1% → 13.0% 로 다른 지표들(10~16%)과 엄격도가 맞고, 2→0 · 4→37.5 ·
    # 5→56.3 · 6→75 가 된다.
    # ※ 7%(=진입선을 관측 최대 위로)도 검토했지만 그러면 **초고온이 구조적으로 불가능해진다**
    #   (§4-1 이 6개 지표에서 걷어낸 '닿을 수 없는 임계값' 과 같은 모양). 관측 최대를 진입선에
    #   놓는 게 두 함정 사이의 자리다. 표본이 23일뿐이라 여전히 잠정 — 6% 위가 나오면 다시 볼 것.
    "bestseller_finance_ratio": {"kind": "fixed", "threshold": 7.33, "floor": 2.0},
    ...
    #
    # 2026-07-29: 초고온 진입선을 +10%p → **+15%p** 로 올린다(ceiling 20 → 26.667).
    # 초고온이 19.5% 로 지표 중 두 번째로 헐거웠다. 26.667 이면 8.6% 가 된다.
    # **floor 는 −20 을 유지한다.** 진입선을 ±30 대칭으로 맞추는 안도 검토했는데, 관측
    # 최저가 −23.3%p 라 floor −30 이면 과열도가 0에 영원히 못 닿아 **구조적 하한 11.1**
    # 이 생긴다(가중치 2.0 이 종합점수에 상시 얹힌다). 스프레드도 그 안은 24.32 인데
    # floor 유지 안은 30.74 로 낫다. 대칭성(0%p = 눈금 한가운데)은 이번에 포기한다 —
    # 진입선을 올리는 이상 셋(floor·ceiling·진입선) 중 하나는 내줘야 하고, 하한을 만드는
    # 쪽보다 대칭을 내주는 쪽이 싸다.
    "kospi_asia_relative_strength": {"kind": "fixed", "threshold": 26.6667, "floor": -20.0},
    ...
    # ※ 2026-08-01: ceiling 75 → **85**. 눈금 자체가 틀려서가 아니라 **아래 축이 움직여서**다.
    #    같은 날 김프 축을 −2~+7 floor-ceiling 으로 바꾸면서(fetch_upbit_speculation.py)
    #    종합 raw 중앙이 24.3 → 34.9 로 올랐고, 75 를 그대로 두면 위 문단이 잡은 목표
    #    11.2% 가 **16.6%** 로 밀린다. 목표치가 결정이 아니라 부작용으로 바뀌는 셈이다.
    #    85 면 초고온 11.6% 로 되돌아오고 천장 포화도 3.7% → 1.6% 로 내려간다(포화는 이
    #    파일이 buffett·kospi_high_gap·kospi_volume_surge 에서 반복해 걷어낸 문제다).
    #    후보 실측(379일, 새 김프 축 기준): 80 → 초고온 14.0%·포화 2.6% · 85 → 11.6%·1.6% ·
    #    90 → 9.2%·1.3%. 90 은 다른 지표 대역(10~16%) 아래로 빠져서 뺐다.
    #    ⚠️ **두 변경은 세트다.** 김프 축을 되돌리면 이 85 도 75 로 함께 되돌릴 것.
    #    ⚠️ 표본이 379일(코인 한 사이클)뿐이다. 다음 상승장이 오면 다시 잡을 것.
    "upbit_speculation_index": {"kind": "fixed", "threshold": 85.0, "floor": 0.0},
    # github_trading_bot_repos: 첫 관측값 하나로 잡았던 150은 **너무 낮았다** — 실측
    # 중앙값이 117이라 13일 중 31%가 천장 포화, 54%가 초고온이었다(다른 지표와 반대 방향의
    # 사고다). floor 40 / ceiling 225(실측 p20/p88)로 다시 얹는다.
    # 2026-08-01: ceiling 을 220 으로 조금 내려 초고온 진입선을 178.75 → **175건**에 놓는다.
    # 관측(21일, 75~183) 안이라 정상 작동하고 초고온이 9.5% → 14.3% 가 된다. 표본이 21일뿐이라
    # 여전히 잠정.
    "github_trading_bot_repos": {"kind": "fixed", "threshold": 220.0, "floor": 40.0},
    ...
    # foreign_sell_at_high: 지수가 52주 고점 **−8%**(fetch 스크립트의 HIGH_GAP_GATE) 이내일
    # 때의 외국인 순매도, 최근 5거래일 누적(억원). 고점권이 아니거나 외국인이 순매수면 0 이다
    # (스크립트가 이미 게이트를 적용해 저장하므로 여기 값은 언제나 0 이상 — floor 를 안 준다).
    #
    # 2026-07-31 신설. 자리는 individual_net_buy 에서 물려받았고, 그 지표는 소스(네이버 금융
    # 스크래핑)가 robots.txt 로 막혀 있어 내렸다. 왜 주체를 개인에서 외국인으로 바꾸면서
    # 게이트가 필요했는지는 fetch_foreign_sell_at_high.py 머리말 참고.
    # 가중치는 2.5 를 물려받았다가 2026-08-01 재보정에서 1.5 로 내렸다(indicator_weights.py).
    #
    # threshold 1조는 **창을 어떻게 잡아도 같은 값**이 나왔다(ECOS 실측):
    #   최근 5년 도달률 6.7% · 20년 도달률 4.3% — 둘 다 put_call(5%) 언저리의 엄격도.
    # 연도별로 갈리는 모습이 이 지표의 존재 이유다:
    #   2022 하락장 0.0% · 2023 3.7% · 2024 2.9% · 2025 8.7% · 2026 27.7%
    # 폭락장에서 0 이고 외국인이 대거 이탈하는 해에만 켜진다.
    # ※ 1조는 progress 100 지점이다. 카드가 "기준선"으로 적는 **초고온 진입선(progress 75)은
    #   7,500억원**이다 — PR #72 가 그 둘을 통일해 뒀으니 '진입선'이라는 말은 75 쪽에만 쓸 것.
    #
    # ⚠️ **값이 0 인 날이 79%(최근 5년, −8% 게이트 실측 1,250일)** 인데 이건 정보 손실이
    # 아니다. market_actions 의 옛 공식이 86% 를 0 으로 뭉갰던 것과는 다르다 — 그쪽은 실제
    # 이벤트가 있던 날까지 버렸고, 이쪽의 0 은 "고점권이 아니거나 외국인이 사는 중" 이라는
    # 사실 자체다. 연속 계기가 아니라 조건이 켜졌을 때만 울리는 지표로 읽어야 한다.
    # (게이트가 −5% 이던 시절 주석엔 86% 라고 적혀 있었다. 넓히면서 79% 가 됐다.)
    "foreign_sell_at_high": {"kind": "fixed", "threshold": 10_000.0},
    # limit_up_breadth: 코스피에서 **종가가** 10% 넘게 오른 종목의 비율(%).
    # 자리와 카드는 investor_deposit(고객예탁금)에서 물려받았다 — 그쪽 소스가 네이버 금융
    # 스크래핑이었고 finance.naver.com 의 robots.txt 가 전면 금지라 내렸다.
    #
    # **비율이라 고정 눈금을 쓴다.** 개수로 재면 상장 종목이 늘기만 해도 값이 올라(2021년
    # 2,300개대 → 2,700개대) 임계값이 금방 낡는데, 분모가 전체 종목 수라 그 증가가 상쇄된다.
    #
    # **초고온 진입선을 2.0% 로 잡는다(2026-08-01).** 눈금은 상한(=과열도 100)으로 적는
    # 자리라 2.0 ÷ 0.75 를 그대로 쓴다 — 숫자 2.667 을 적어 두면 "왜 2.667 인가"가 남는다.
    # 단계 경계는 이 한 줄에서 전부 따라 나온다(상온 1/4·고온 1/2·초고온 3/4 지점).
    #
    # 실측 1,342거래일(2021-02~2026-07-31) 기준 이 선의 뜻:
    #   저온 45.5% · 상온 38.5% · **고온 10.5%** · 초고온 5.5%(74일)
    #   중앙 0.737% → 과열도 28 · p95 2.08% → 과열도 78 · 최대 19.66%
    #   단계 경계: 상온 0.667%(732일) · 고온 1.333%(215일) · 초고온 2.0%(74일)
    #
    # **고온 구간이 10% 근처로 나오게 잡은 선이다.** 초고온을 2.5% 로 올려 보니 고온이
    # 5.7% 까지 얇아졌다 — 선형 눈금에서는 초고온 진입선 하나가 나머지 경계를 다 끌고
    # 가므로(상온·고온이 그 1/3·2/3 지점) 고온만 따로 넓힐 수가 없다. 2.0% 에서는 고온
    # 10.5% 가 그냥 떨어져서 눈금을 꺾는 장치(kospi_high_gap 식 piecewise)를 안 넣어도
    # 된다. 2.5% 를 유지하면서 고온을 10% 로 만들려면 그 장치가 필요했다.
    #
    # 2021 개인 광풍도 초고온에 닿는다(광풍해 최대 3.36% > 2.0%, 2021년 초고온 12일).
    # 초고온 진입선을 5.0% 로 올려 봤더니 광풍 최대가 과열도 50 에서 멈춰 폭락만 잡는
    # 카드가 됐다 — 광풍을 잡으려면 선이 3.36% 아래여야 하고, 그러면 폭락일도 같이
    # 들어온다. 이 지표로 둘을 가르는 눈금은 없다(아래 '변동성에 가깝다' 참고).
    # 초고온 연도별: 2021 12일 · 2022 1일 · 2023 5일 · 2024 4일 · 2025 10일 · 2026 42일.
    #
    # 앞선 눈금들: 상한 5.0(장중 고가 시절) → 2.5(종가로 바꾸며) → 2.667.
    # 같은 날 기준 장중 6.15% vs 종가 1.17% 로 값이 통째로 작아진다. 세는 기준을 바꾸면
    # 눈금도 반드시 다시 잡을 것 — 카드 설명의 '평소 N% 안팎'도 같이 움직인다.
    # floor 는 계속 0 이다. 실측 중앙이 0.74% 이고 0 인 날이 7일뿐이다.
    #
    # ⚠️ **이 지표는 과열보다 변동성에 가깝다.** 국면별 실측 중앙: 하락장 0.53% · 평범
    # 0.63% · 회복 0.73% · 강세 0.73% · 2021(광풍 포함) 0.76% · **2026 폭락 1.37%**.
    # 평범한 국면끼리는 순서가 맞지만(광풍 > 강세 > 회복 > 평범 > 하락장) 폭락만 따로 튄다.
    # 급등 종목은 시장이 어느 쪽으로든 크게 흔들릴 때 함께 늘기 때문이다. 상승−하락 상쇄·
    # 고점권 게이트·상한가 유지율 등 10형태를 시험하고 가중치 46,225개 조합을 전수
    # 탐색했지만 광풍과 폭락을 가르는 조합이 없었다(그 탐색에서 21%가 '분리'에 성공 —
    # 통과율 자체가 신호가 아니라는 증거다).
    #
    # ⚠️ **가중치 1.5 는 상한 2.5% 로 잰 값이다**(전수 검사 3: +25.3 p=0.00). 눈금을 이만큼
    # 올리면 지표가 거의 늘 0 근처라 그 측정은 다시 해야 한다 — indicator_weights.py 를
    # 함께 볼 것.
    "limit_up_breadth": {"kind": "fixed", "threshold": 2.0 / 0.75},
