...
INDICATOR_THRESHOLDS = {
    ...
    #
    # ceiling 2.5%: 코스피만 1,341거래일(2021-02~2026-07) 실측으로 전체 도달률 3.1%.
    # 평범한 해는 0.0~2.5% 만 닿는다. 3.0% 는 전체 1.9% 로 빡빡하고, 5.0% 는 0.5% 라
    # 사실상 아무도 안 닿는다. floor 는 0 — 실측 중앙이 0.74% 이고 0 인 날이 7일뿐이다.
    #
    # ⚠️ **장중 고가에서 종가 기준으로 바꾸면서 눈금을 다시 잡았다(5.0 → 2.5).** 같은 날
    # 기준 장중 6.15% vs 종가 1.17% 로 값이 통째로 작아진다. 세는 기준을 바꾸면 눈금도
    # 반드시 다시 잡을 것 — 카드 설명의 '평소 N% 안팎'도 같이 움직인다(2% → 1%).
    #
    ...
    # 그래서 **가중치를 1.0** 으로 낮춰 들여놓는다(예탁금은 3.0 이었다).
    "limit_up_breadth": {"kind": "fixed", "threshold": 2.5},
