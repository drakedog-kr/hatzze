...
INDICATOR_THRESHOLDS = {
    ...
    #
    # 2026-07-29: 초고온 진입선을 3.5조 → **10조**로 올린다(ceiling 67,000 → 153,333).
    # 위 눈금은 **10일 표본**으로 잡은 것이라 실제 분포와 크게 어긋나 있었다. DB 히스토리가
    # 14일뿐이라 수집원(네이버 금융 투자자별 매매동향)에서 1년치를 직접 긁어 재보니
    # 245영업일 기준 **−9.63조 ~ +21.61조**(평균 +1.85조 · 중앙 −0.21조 · p90 +12.50조)로,
    # 옛 ceiling 6.7조는 **1년의 20.8%가 천장 포화**였다. 진입선 3.5조의 도달률도 27.8%로
    # 전 지표 중 가장 헐거웠다.
    # 10조 = 실측 p87 근처라 초고온이 13.1%, 천장 포화가 5.3%가 된다(다른 지표와 같은 엄격도).
    # **floor −60,000 은 유지** — 바닥 포화 6.1%로 적당하고, 낮추면 순매도의 정도를 다시
    # 뭉개게 된다. 그래서 ceiling 이 153,333(=(100,000−0.25×(−60,000))/0.75)이라는 딱 떨어지지
    # 않는 값이 된다. 화면에 나오는 건 진입선(10조) 쪽이니 그쪽을 라운드로 잡는 게 맞다.
    # ※ backtest/current.py 의 이 지표 행은 히스토리 13일을 상수로 놓고 계산해 의미가 없다.
    #   위 수치는 전부 1년 실측 스크레이프 기준이다.
    "individual_net_buy": {"kind": "fixed", "threshold": 153_333.33, "floor": -60_000.0},
