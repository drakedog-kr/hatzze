...
TIGER '전체 분배 내역'(`investments.miraeasset.com/tigeretf/ko/distribution/overall/list.ajax`)이 서버에서
그려져 curl 로 열린다. 지급 건마다 기준일·지급일·분배금이 있어 **달력에 든다.** 스크립트가 매일 최근 열다섯
달치를 받아 지난 1년에 지급이 있는 종목(2026-09-13 실측 212)을 넣는다. 그전엔 연간 표만 받아 달을 몰랐다.
⚠️ 운용사 사이트를 자동으로 받는 것이다. 회원 약관에 복제·제공 금지 조항이 있다(스크립트 머리말).
