...
일찍 돌수록 좋다). 이건 반대다.
  ...
  · 전날 종가를 KRX 에서 직접 받으므로 **08:00 KST 공표 뒤**여야 한다(아래 krx_prev_close —
    `stocks` 표는 읽지 않는다. 그 표를 채우는 `fetch_krx_stocks` 가 08:00 앞에 있어서다).
  · 값이 09:00 에 가까울수록 정보가 많다. 국내 개장 직전 한 시간이 가장 값지다.
  · 2026-09-10 에 자리를 옮겼다 — KRX 게이트 뒤(도달 08:36~09:29)에서 **아침 발송 뒤**(08:35 무렵)로.
    저 자리는 최근 아침 5회 중 3회가 09:00 을 넘겨 아래 장중 가드에 스스로 걸렸다.
