...
⚠️ **발송에 TELEGRAM_SESSION(Telethon)을 쓰지 않는다.** 그건 운영자 개인 계정의 로그인
...
# 2026-09-23 Haiku → Opus 5.5. 구독자에게 그대로 나가는 글이다. 평일 두 편에 수·토·일 한 편씩
# 더 나가고 한 편에 한 번 부른다.
# ⚠️ Opus 도 Sonnet 처럼 전언 지시를 문장마다 따른다. 그래서 _LLM_RULES 의 [표현]을 다시 썼다
#    (common/broadcast_content 의 _LLM_RULES 주석에 실측). 같은 재료로 견주면 Haiku 는 목표가·
#    매수세 같은 주가 표현을 쓰고, 글 머리에 인사말을 붙여 코드가 갈래를 버리거나 다시 만들게
#    했다. 수요일 글은 3번 중 1번 형식을 어겨 통째로 안 나갔다. Opus 는 셋 다 없었다.
#    길이는 Opus 가 같거나 짧다(같은 재료 4회: 개장 전 591 → 541자 · 마감 631 → 478자 ·
#    수 680 → 568 · 토 708 → 627 · 일 698 → 591). 마감이 짧은 건 '세네 갈래'에서 셋을 고른
#    날이 있어서다.
...
def build_evening(score_row: dict, surging: list[dict]) -> str:
    """B · 마감 리포트 — 평일 저녁. [온도] → [오늘의 요약] → [급부상 종목 3] → [CTA].
...
def build_morning(db, llm) -> str:
    ...
    **종목을 말하지 않는다.** 저녁 글이 종목을 맡으므로 아침은 '주제·사건'만 다룬다.
    ...
    # 갈래 문단이 사라졌으면 **키워드를 날것 그대로 싣는다.**
    #
    # 그 문단은 키워드를 갈래로 묶어 읽히게 만든 것이지 없던 정보를 만든 게 아니다. 묶음이
    # 사라졌다고 키워드까지 버릴 이유가 없다 — 나열이 문단보다 못할 뿐 아무것도 없는 것보단
    # 훨씬 낫고, 이 줄이 있으면 해설이 통째로 걸린 날에도 글이 나간다.
    #
    # 실측(2026-07-29): 금지어 하나에 갈래 두 문단이 걸려 아침 글이 통째로 안 나갔다.
    # 그날도 키워드는 멀쩡히 있었다.
    #
    # 날짜를 부르는 말은 **반드시 day 를 쓴다**(bc.morning_day_words). 여기에 "어제"를
    # 박아 두면 집계가 밀린 날 제목은 '어제'를 빼는데 이 줄만 "어제 오간 말"로 남는다 —
...
def build_theme(db, llm) -> str:
    """C · 관심 이동 — 3일마다. [테마 순위·증감] → [무엇이 달라졌나] → [CTA].
    ...
    # 키워드 라벨에는 **집계일을 날짜로 못박는다.** 아침 글과 달리 집계일이 진짜 어제여도
    # '어제'를 쓰지 않는다(bc.morning_day_words 를 안 부르는 이유).
    #
    # 이 글은 재료가 두 기간이다 — 테마·순위·%p 는 전부 **최근 3일 평균**이고 키워드만
    # 하루치다. 라벨에 '어제'가 있으면 모델이 그 말을 3일 창 숫자에까지 옮겨 붙여, 3일
    # 평균을 하루치인 양 말한다. 집계일이 정말 어제인 날조차 그랬다(30회 실측:
    # "어제는 전체 24.1%를 차지했지만" · "어제 점유율은 4.6%로 전날보다 2.8%포인트 올랐고"
    # — 24.1%·4.6% 는 3일 평균이고 +2.8%p 는 그 이전 창과의 차이다). 날짜를 박은 30회에는
    # 그런 문단이 없었고, 모델은 아예 키워드에 날짜를 안 붙였다(날짜말 1/79 문단).
    #
    # 집계일이 어제가 아닌 날은 라벨이 그냥 거짓말이었다. 집계일이 오늘이던 2026-07-28 에
    # "[어제 키워드]"로 30회를 돌리니 21회가 오늘치 집계를 본문에서 '어제'라고 불렀다.
    # 그중 5회는 하루치 키워드를 반씩 갈라 어제·오늘 이야기를 지어냈다("어제는 DUV 노광장비
    # 같은 미시적 주제가 떠돌다가 오늘은…"). 날짜를 박은 30회는 0회였다.
    #
    # 탈락률은 안 갈렸다(어제 27/180 · 날짜 20/180, Fisher p=0.35). 라벨만 바꿨으니
    # 그럴 자리도 아니다.
    kdate, keywords = bc.load_keywords(db)
    digest = "\n".join(
        dropped_line
        ...
        # 키워드가 비면 라벨만 남은 빈 줄이 나가므로 줄째로 뺀다(예전 `[...] or []` 는
        # 원소가 있는 리스트라 언제나 참이었다 — 아무것도 막지 못했다).
        + ([f"[{bc.day_label(kdate)} 키워드] {' · '.join(k for k, _ in keywords)}"] if keywords else [])
    )
...
def build_weekly(db, llm) -> str:
    """D · 주간 결산 — 주 1회. [주간 최다 종목 3] → [테마] → [마무리] → [CTA].

    **온도를 넣지 않는다**(운영 결정). 온도는 어디서든 만들 수 있지만 316개 채널을 매일
