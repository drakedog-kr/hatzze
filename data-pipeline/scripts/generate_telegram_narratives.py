...
있으면 대목을 새로 낼 것 — 재료가 없는 대목은 길이를 올려도 안 늘어난다(BRIEF_*_LEN 주석).
  ...
  ① 분위기   [전체] 낙관도 + [낙관도 추이]           105~118자 · 2문장
  ② 테마 지형 [테마별] 건수·순서 + 반대편 [화제어]    155~170자 · 2문장
  ③ 이야기   [오간 이야기] 발췌 + [화제 종목]        215~230자 · 3문장
...
- 대화를 **가장 많이 채운 주제**는 써도 됩니다([화제어] 상위의 실적호조·AI투자 수준).
...
[이번 대목 — 테마 지형]
[테마별]과 [최근 화제어]를 근거로, **이 낙관이 어디에 몰려 있고 어디가 다른지**를
**두 문장**으로 쓰세요. 앞 대목이 생태계 전체의 온도 하나를 말했으니, 여기는 그 온도가
고르지 않다는 것을 보입니다.

- **이 대목의 본론은 '부피'입니다.** 화면의 테마 막대는 테마마다 비관↔낙관 **비율**만
...
- **[최근 화제어]에서 분위기와 반대 방향인 말**(하락세·실적부진 같은 것)이 상위에 있으면
  집어 주세요. "낙관 우세지만 한쪽 이야기만 오간 건 아니다"가 이 대목의 마무리입니다.
  반대 방향의 말이 상위에 없으면 억지로 만들지 말고 그냥 빼세요.
