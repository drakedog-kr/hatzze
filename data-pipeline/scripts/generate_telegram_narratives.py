...
예외는 총평 셋째 대목 하나다(2026-09-24 · BRIEF_NEWS_COMMON): 목표주가 상향·하향 소식은 숫자
없이, 실적·업황 전망은 누가 봤는지를 붙여 전한다. 주가 예측은 거기서도 막는다.
...
# ── 센티먼트(낙관도)만 쓰는 창 ─────────────────────────────────────────────
#
# 위 WINDOW_DAYS 는 발췌·종목 리포트·화제어가 쓰는 3일이다. 낙관도 숫자는 **다른 창**을
# 쓴다 — 오늘과 어제, 둘. 예전엔 "기준일을 뺀 앞 3일"이었는데 그러면 오늘 들어온 글이
# 숫자에 하나도 안 들어가서, 화면이 사흘 전 분위기를 오늘 것처럼 보여 줬다. 실측(2026-09-06,
# 거래일 38일): 하루치 낙관도와 그날 코스피 등락의 상관이 +0.62 인데, 앞 3일 창은 −0.21 로
# 방향이 반대였다. 코스피 −10.8% 였던 날 화면은 낙관이었다. 오늘+어제는 +0.38 이다.
#
# ⚠️ **프론트 SENTIMENT_WINDOW_DAYS(lib/telegram-data.ts)와 같은 값·같은 규칙이어야 한다.**
#    총평 문장이 인용하는 낙관도와 옆 막대가 같은 날들을 봐야 한다.
# ⚠️ 표본이 얇은 날(주말)은 창을 뒤로 하루씩 넓힌다 — 판정을 보류하는 날은 없다. 문턱은
#    오늘+어제 건수 실측(56일)에서 중앙 7,957 · 하위¼ 2,790 이었고, 2,000 이면 넓히는 날이
#    8일(대부분 주말·수집 첫날)이다.
SENTIMENT_WINDOW_DAYS = 2
SENTIMENT_WINDOW_MAX_DAYS = 4
SENTIMENT_MIN_MESSAGES = 2000


def sentiment_window(count_by_date: dict[str, int], latest: str, floor: int) -> list[str]:
    """낙관도가 볼 날짜 목록(오래된→최신). 기준일과 어제, 표본이 얇으면 하루씩 더 뒤로.

    프론트 lib/telegram-data.ts 의 sentimentWindow 와 같은 규칙이다(손으로 맞춘 사본).
    """
    base = date.fromisoformat(latest)
    days = [(base - timedelta(days=k)).isoformat() for k in range(SENTIMENT_WINDOW_DAYS)]
    while sum(count_by_date.get(d, 0) for d in days) < floor and len(days) < SENTIMENT_WINDOW_MAX_DAYS:
        days.append((base - timedelta(days=len(days))).isoformat())
    return sorted(days)
...
# 총평 셋째 대목(Opus 5.5)만 쓰는 COMMON. 나머지 대목·종목 요약·테마 요약은 COMMON 그대로다.
# 2026-09-24 결정: 목표주가는 '올렸다·내렸다'는 소식까지 숫자 없이, 실적·제품 가격·업황 전망은
# 누가 그렇게 봤는지를 붙여 전한다. 주가 예측과 증권사가 주가를 두고 한 평가는 계속 막는다.
# 옛 줄("목표가 · 상승/하락 예측 금지")을 Haiku 는 반쯤 무시해 이런 문장이 나가고 있었고, Opus 는
# 글자 그대로 따라 "메모리 가격이 오를 것"까지 한 번도 안 썼다(같은 재료 4회씩 Haiku 40문장 · Opus 0).
# 바꾼 뒤 4회: "메리츠증권이 삼성전기의 목표주가를 올렸고 … 웃돌 것으로 봤습니다"처럼 쓰고,
# 목표주가 금액·주가 예측은 0건이었다.
_BAN_OLD = "- 매수/매도/투자 권유·신호, 목표가, 주가 상승/하락 예측('오를 것/내릴 것/앞으로')."
_BAN_NEW = (
    "- 매수/매도/투자 권유·신호, 주가가 오를지 내릴지 예측('주가가 오를 것/내릴 것').\n"
    "- 증권사가 주가를 두고 한 평가('저평가', '과도한 조정', '매수 의견', '비중확대', '상승 여력')와\n"
    "  목표주가 **숫자**."
)
TARGET_AND_OUTLOOK = """
[전망과 목표주가]
- 실적·제품 가격·업황 전망은 쓸 수 있습니다. 다만 **누가 그렇게 봤는지를 붙여** 씁니다.
  예: "○○는 △△ 가격이 더 오를 것으로 봤습니다."
- 목표주가는 **올렸다·내렸다는 소식까지만, 숫자 없이** 씁니다.
  예: "○○증권이 △△의 목표주가를 올렸습니다." (X) "목표주가를 □□원으로 제시했습니다."
"""
assert COMMON.count(_BAN_OLD) == 1 and COMMON.count("\n[출력]") == 1
BRIEF_NEWS_COMMON = COMMON.replace(_BAN_OLD, _BAN_NEW).replace("\n[출력]", TARGET_AND_OUTLOOK + "\n[출력]")

# ⚠️ 예문에 시점·숫자를 적지 말 것. 예전 예문 "이번 주 중반 한 차례 식었다가"(미장은 "주 후반
#    한 차례 식었습니다")를 모델이 재료 대신 그대로 옮겼다 — 2026-09-15(화) 미장 총평이
#    "주 후반 톤이 내려앉은"이라고 썼다. 근거는 여기(문자열 밖)에 둔다 — 문자열 안에 적으면
#    모델에게 전송돼 그 말을 되레 상기시킨다.
BRIEF_TONE_SYSTEM = COMMON + f"""

[이번 대목 — 오늘의 분위기]
**오늘 하루** 이 생태계의 분위기가 어느 쪽인지를 먼저 쓰고, 그게 이 기간의 다른 날과 어떻게 다른지를
...
# ⚠️ 부피 비교의 셈은 모델에게 맡기지 않는다 — 아래 좋은 예의 틀에 숫자를 넣다 틀린다
#    (2026-09-15 저녁: 548건 대 537건을 "합친 것보다 훨씬 많습니다"). 비교문은 digest 의
#    [부피 비교] 줄이 코드로 만들어 준다(volume_comparison_line). 사고 사례는 문자열 밖에 둔다.
...
- **[오늘 새로 오른 화제어]는 앞 3일엔 거의 없다가 오늘 올라온 말입니다.** 목록을 그대로
...
- **두 문장 모두 70자 안팎으로 채웁니다.** 첫 문장이 낱말 나열로 끝나면 이 대목이 길이를
  ...
  좋은 예의 뼈대: "3일 전엔 없던 ○○ 이야기가 오늘 새로 올라왔고, ○○ 쪽 언급이 거기에
  붙었습니다."
...
BRIEF_NEWS_SYSTEM = BRIEF_NEWS_COMMON + f"""
...
#
# ⭐ **넷째 대목의 날짜 오기는 프롬프트가 아니라 발췌가 만든다.** DART 알림 한 건에
#    날짜가 둘씩 들어 있다 — 글머리의 **올린 시각**과 본문의 `예정일자`다.
#
#        2026.09.04 16:09:22 기업명: SKC … 예정일자 : 2026-11-24 …
#        └ 글 올린 때(앞날 아님)                  └ 진짜 앞날
#
#    모델은 앞의 '09'와 뒤의 '24'를 붙여 **"9월 24일"**을 썼다(2026-09-07 실측). 같은
#    실행에서 지엔씨에너지의 9월 10일이 다른 발췌의 '18일'과 섞여 "9월 18일"이 되기도
#    했다. 날짜를 고르는 일을 모델에게 시키지 말고, 재료에서 하나로 만들어 준다.
#
# ⭐ 형식도 같이 맞춘다. `예정일자 : 2026-11-24` 를 그대로 두면 모델이 그 칸 이름째
#    옮겨 적는다("…공시가 예정일자 2026년 11월 24일로 올랐습니다", "예정일자
#    2026-11-24일 이루는 것으로" — 10회 중 2회). 문장에 쓸 꼴로 미리 적어 두면
#    베껴 쓰기만 하면 된다.
#
# ⚠️ 올린 시각은 **`기업명` 이 뒤따르는 것만** 지운다. 본문 속 날짜를 무턱대고 지우면
#    "9월 21일 적용 예정" 같은 진짜 앞날까지 사라진다.
_DART_STAMP = re.compile(r"\d{4}[.\-]\d{2}[.\-]\d{2}\s+\d{2}:\d{2}(?::\d{2})?\s+(?=기업명)")
# `예정일자 : 2026-11-24` · `기준일자 : 2026-10-13` · `발표일자 : 2026.09.04` …
_DATE_FIELD = re.compile(r"([가-힣]{1,4})일자\s*[:：]\s*(\d{4})[-.](\d{1,2})[-.](\d{1,2})")
# 정리된 발췌가 DART 알림인지. schedule_lines 가 '공시는 한 건만' 주의를 붙일지 정한다.
_DART_HEAD = re.compile(r"기업명\s*[:：]")


def schedule_excerpt(text: str, base: date) -> str:
    """일정 발췌 한 줄. 날짜를 하나로 정리한 뒤 자른다(위 주석의 실측 참고).

    `readable_counts` 로 자릿수를 묶는 것은 [오간 이야기] 발췌와 같다 — 그쪽에서
    "24,070,000주"가 "24억 주"로 나온 자리다.
    """
    flat = _DART_STAMP.sub("", " ".join((text or "").split()))

    def _korean(m: "re.Match[str]") -> str:
        label, year, month, day = m.group(1), int(m.group(2)), int(m.group(3)), int(m.group(4))
        when = f"{month}월 {day}일" if year == base.year else f"{year}년 {month}월 {day}일"
        return f"{label}일 {when}"

    return readable_counts(_DATE_FIELD.sub(_korean, flat))[:SCHEDULE_CHARS]
    ...
    **재료 옆에** 적으니 그제야 멎었다. 규칙이 안 먹으면 프롬프트가 아니라 재료 옆을 본다.
    ...
    셋 다 실측으로 필요한 것들이다(2026-09-06):
      ...
      ② 날짜 — 같은 애플 행사를 9월 9일이라 적은 글과 9월 10일이라 적은 글이 같은 기간에
         둘 다 있었다. 그대로 옮기면 둘 중 하나는 거짓이 된다. 한 발췌 **안**에서 날짜가
         섞이는 것은 이 줄로 안 멎어서 재료 쪽에서 풀었다(schedule_excerpt).
    ...
    out += [
        ...
        "  ※ 한 발췌에 날짜가 여럿이면 **그 일에 붙은 것만** 쓰고, 다른 발췌의 날짜를 이"
        " 회사에 끌어다 붙이지 마세요.",
    ]
    # ⚠️ **공시 이야기는 공시가 있는 날만 적는다.** 국장 발췌에는 DART 알림이 섞이지만
    #    미장에는 한 건도 없다. 없는 재료를 두고 "그중 하나만 쓰라"고 적으면 모델이 이
    #    블록에 쓸 게 없다고 읽는다(미장 10회 중 5회가 일정 대신 분위기 요약이었다).
    if any(_DART_HEAD.search(t) for t in picked):
        out.append(
            "  ※ 공시 발췌('예정일 9월 10일'처럼 적힌 것)는 **한 건만** 쓰세요. 날짜가 확실해"
            " 집어 쓰기 좋지만 둘을 나란히 놓으면 이 대목이 매일 공시 목록이 됩니다."
            " 남는 자리는 행사·상장·편입 같은 다른 갈래에서 집으세요."
        )
    out.append(
...
def base_day_block(
    ...
    if spikes:
        ...
        # 틀린 예("금리 약세")는 재료에 안 적는다 — 그 말을 되레 상기시킨다.
        lines.append("  ※ 화제어는 적힌 뜻 그대로 쓰세요. 두 낱말을 하나로 합치거나 바꿔 부르지 마세요.")
    lines.append(
        "[오늘 화제어] 많이 나온 순: "
...
def build_brief_digest(db, latest: str, msgs: list[dict]) -> str | None:
    ...
    """
    # ⚠️ 두 창이 있다. `since..end`(기준일 뺀 앞 3일)는 **화제어**가 쓰는 창이고, 낙관도와
    #    테마별 톤은 아래 `sent_days`(오늘+어제, 얇으면 넓힘)를 쓴다 — SENTIMENT_WINDOW_DAYS 주석.
    ...
    count_by_date = {r["date"]: r["message_count"] for r in all_sent if r["scope"] == "overall"}
    sent_days = sentiment_window(count_by_date, latest, SENTIMENT_MIN_MESSAGES)
    sent = [r for r in all_sent if r["date"] in set(sent_days)]
