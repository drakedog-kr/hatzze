...
INDICATOR_META = {
    ...
    # 과열)의 같은 어법을 쓰므로 문구도 짝을 맞춘다 — "…대비하는 사람이 적습니다".
    # 목적어만 다르다: VKOSPI 는 '하락', 이쪽은 '충격'이다. 환율 변동성이 커지는 건
    # 지수가 빠져서가 아니라 대외 충격(금리·유가·지정학)이 들어올 때라서, 여기에
    # '하락'을 쓰면 무엇에 대비한다는 건지가 안 맞는다.
    "description_beginner": "환율이 잔잔할수록 충격에 대비하는 사람이 적습니다",
    "unit": "%",
}
