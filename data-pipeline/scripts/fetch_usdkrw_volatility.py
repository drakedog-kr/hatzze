...
INDICATOR_META = {
    ...
    # 과열)이라 '대비' 라는 같은 동사를 쓴다.
    #
    # ⚠️ 목적어가 '하락'인 것은 **주가 하락**을 가리킨다(환율 하락이 아니다). 이 화면이
    # 통째로 주식 과열도라 그 독법이 성립하지만, 카드 제목이 '원/달러 환율 변동성'이라
    # 곧이곧대로 읽으면 환율이 내리는 쪽으로 붙는다 — 환율 하락은 원화 강세라 위험이
    # 아니고, 이 지표가 커지는 실제 계기는 대외 충격(금리·유가·지정학)이다.
    # 2026-08-04 에 이 점을 짚었고, 그럼에도 이 조합으로 가기로 정해졌다. 바꾸려면
    # 'VKOSPI=하락 / 환율=충격' 으로 목적어만 맞바꾸면 된다(폭은 둘 다 그대로다).
    #
    # ⭐ **각주는 4열 하한(뷰포트 1400px · 칸 안쪽 237px)에서 재야 한다.** 1440 에서
    # 1줄이어도 1400 에서 2줄이 되는 문구가 있다. 두 줄이 되면 각주 위 구분선이 이웃 칸보다
    # 19px 높이 올라가 한 행의 밑선이 어긋난다. 실측(2026-08-04, 칸 폭 237):
    #   "환율이 잔잔할수록 아무도 대비하지 않는다는 뜻입니다"  2줄 ← 1440 에선 1줄이라 안 걸렸다
    #   "환율이 잔잔할수록 충격에 대비하는 사람이 적습니다"    1줄 폭 233 (여유 4)
    #   "잔잔할수록 하락에 대비하는 사람이 적습니다"           1줄 폭 200 (여유 37) ← 채택
    "description_beginner": "잔잔할수록 하락에 대비하는 사람이 적습니다",
    "unit": "%",
}
