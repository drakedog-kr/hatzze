...
## 세 대목이 맡는 재료 (국내와 셋째가 다르다)

  ① 분위기   [전체] 낙관도 + [낙관도 추이]                       100~115자 · 2문장
  ② 테마 지형 [테마별] 점유율 + [미장 쏠림 화제어]                 150~170자 · 2문장
  ③ 이야기   [오간 이야기] 발췌 + [화제 종목] + **[국내로 옮겨붙은 것]** 185~210자 · 3문장

⭐ 셋째 대목에 **함께 언급된 국내 종목**을 넣는 것이 이 총평의 존재 이유다. 미국 소식이
어느 국내 종목으로 옮겨붙는지는 한국 채널만 말하고, 이 화면의 다른 카드가 이미 그걸
표로 보여 준다. 문장이 그 표를 가리켜 주지 않으면 총평은 영어권 서비스의 요약과 같아진다.
...
[이번 대목 — 지금 오가는 이야기]
[오간 이야기] 발췌·[화제 종목]·[국내로 옮겨붙은 것]을 근거로, 이 채널들에서 **무슨
이야기가 오가고 있는지**를 **두세 문장**으로 쓰세요. 앞 두 대목이 분위기와 테마를 맡았으니
**여기는 내용을 맡습니다** — 종목명과 사건을 구체적으로 집으세요.

- ⚠️⚠️ **반드시 [국내로 옮겨붙은 것]에서 한 짝을 집어 한 문장을 쓰세요.** 이 대목의
  존재 이유입니다. 미국 소식이 어느 국내 종목과 함께 오르내리는지는 한국 채널만 말합니다.
  그 짝을 안 쓰면 이 문단은 어디서나 볼 수 있는 미국 시황 요약이 됩니다.
  ⚠️ 다만 **함께 불렸다는 것이지 두 회사가 실제로 엮여 있다는 뜻이 아닙니다.**
  "~와 함께 거론됐습니다", "~ 이야기에 ~가 같이 불렸습니다"처럼 **함께 언급됐다**고만
  적으세요. 공급 계약·수혜·협력 같은 관계를 지어내면 안 됩니다.
  ⚠️⚠️ **적힌 두 이름을 짝 그대로 쓰세요.** 한쪽만 쓰거나, 목록에 없는 다른 종목과
  새로 짝지으면 안 됩니다. 나쁜 예: "삼성전자의 HBM4 수율 달성 소식에 RF머트리얼즈가
  같이 언급되었습니다" — 목록의 짝은 **코히어런트 × RF머트리얼즈**인데 미국 쪽을 버리고
  발췌에 있던 다른 회사를 붙였습니다. 데이터에 없는 짝을 만든 것입니다.
  ⚠️⚠️ **앞 문장의 이야기에 그 짝을 이어 붙이지도 마세요.** 나쁜 예: "데이터센터 건설
  제약 우려 속에서 코히어런트와 RF머트리얼즈가 함께 언급되었습니다" — 그 둘이 왜 같이
  불렸는지는 데이터에 없는데 "~ 속에서"가 없는 이유를 만들어 냅니다.
  좋은 예: "코히어런트와 RF머트리얼즈가 같은 글에서 함께 거론되기도 했습니다."
  그 짝은 **독립된 문장**으로, 배경 설명 없이 적으세요.
  ⚠️ 배수(74.8배 같은 숫자)는 쓰지 마세요. 옆 표가 그 숫자를 찍습니다.
- 그 짝 문장 말고 **미국 쪽 이야기도 한 가지 이상** 집으세요(발췌·화제 종목에서).
...
def build_brief_digest(db, latest: str, msgs: list[dict], name_of: dict[str, str]) -> str | None:
    ...
    # ── ⭐ 국내로 옮겨붙은 것 (이 총평의 존재 이유) ──────────────────────────
    head = (
        db.table("telegram_us_comention")
    ...
    if head:
        pairs = (
            db.table("telegram_us_comention")
            .select("ticker,stock_code,pair_count,channel_count,lift")
            .eq("as_of_date", head[0]["as_of_date"])
            .eq("window_days", head[0]["window_days"])
            .order("lift", desc=True)
            .limit(COMENTION_ROWS)
            .execute()
            .data
        ) or []
        if pairs:
            codes = sorted({p["stock_code"] for p in pairs})
            kr_name = {
                s["code"]: s["name"]
                for s in (db.table("stocks").select("code,name").in_("code", codes).execute().data or [])
            }
            lines += [
                "",
                f"[국내로 옮겨붙은 것] 최근 {head[0]['window_days']}일, 미국 종목 이야기에 "
                "**같이 불린** 국내 종목입니다",
                "  ※ 함께 언급됐다는 뜻일 뿐, 두 회사가 실제로 엮여 있다는 근거가 아닙니다.",
                # ⚠️ 이 두 줄은 프롬프트가 아니라 **digest** 에 있다. 같은 금지를 시스템
                # 프롬프트에 세 번 다르게 적었는데도 모델이 매번 위 발췌의 사건에 이 짝을
                # 이어 붙였다("데이터센터 건설 제약 논의에서 …가 함께 거론되었습니다").
                # 재료 옆에 적으니 그제야 멎었다 — 규칙이 안 먹으면 프롬프트 말고 재료를 볼 것.
                "  ※ 이 목록은 **위 [오간 이야기] 발췌와 아무 관계가 없습니다.** 창도 다르고"
                "(14일 vs 3일) 계산도 별개입니다.",
                "  ※ 그러니 이 짝을 위 발췌의 사건과 이어 붙이지 마세요. 어느 글에서 왜 같이"
                " 불렸는지는 이 데이터에 없습니다.",
            ]
            for p in pairs:
                lines.append(
                    f"- {name_of.get(p['ticker'], p['ticker'])} × "
                    f"{kr_name.get(p['stock_code'], p['stock_code'])} "
                    f"(같은 글 {p['pair_count']}회 · {p['channel_count']}개 채널)"
                )
