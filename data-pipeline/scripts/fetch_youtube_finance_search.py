...
INDICATOR_META = {
    ...
    "category": "감성",
    # "조회수가 확 뛰면 콘텐츠가 빠르게 퍼지는 것입니다" 였다(~2026-08-04). 유튜브
    # 안에서 벌어지는 일만 말하고 **그게 시장과 무슨 상관인지**를 안 짚어서, 과열도
    # 지표로 왜 세는지가 카드에서 끊겼다. 다른 감성 카드들처럼 '관심의 쏠림'으로 잇는다.
    #
    # ⚠️ 끝을 "…쏠렸다는 뜻입니다"로 뒀다가 줄였다. 4열 하한(뷰포트 1400px · 칸 안쪽
    # 237px)에서 폭 226 이라 **2줄로 넘어갔다** — 1440px 에서 1줄이라 처음엔 못 봤다.
    # 각주가 두 줄이 되면 그 위 구분선이 이웃 칸보다 19px 올라가 한 행의 밑선이 어긋난다.
    # 지금은 폭 213(여유 24)이고, 어미도 VKOSPI·환율 카드와 같은 꼴이 됐다.
    "description_beginner": "평소보다 많이 볼수록 주식에 관심이 쏠렸습니다",
    "unit": "회",
}
