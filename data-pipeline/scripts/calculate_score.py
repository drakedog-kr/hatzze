...
# 중앙값이 53이다.
#
# 이 눈금에서 2025-07~2026-08 구간은: 고점 기간 최고치 86/95/87(세 고점 모두 75 이상),
# 저점 41/40/26, 폭락 직후 오늘 24, 중앙 52, 국면 비중 저온 2% · 상온 48% · 고온 31% ·
# 초고온 20%. (2026-08-06 재측정. 검사 3 가중치를 반영한 백테스트 원점수 기준이라, 예전에
# 적혀 있던 79/88/86 · 저점 47/45/35 · 상온 37/고온 44/초고온 19 와는 다르다.)
#
# ⚠️ **아래 두 줄은 2026-08-01 재측정에서 깨졌다. 표본이 늘어난 결과다.**
#   (1) 예전엔 "향후 90일 최대낙폭이 4분위에서 −1.1 / −2.3 / −2.9 / −4.3%로 단조롭게
#       나빠진다"고 적혀 있었다. 07-22 이후 코스피가 5,594(전고점 −38.6%)까지 더 빠진
#       구간이 표본에 들어오자 **단조성이 사라졌다** — 실측 −2.3 / −1.7 / −2.7 / −15.4 로
#       Q1~Q3 이 구분되지 않고 Q4 에서만 절벽이 생긴다. 십분위로 보면 1~8 분위가
#       −0.8~−4.4% 에서 순서 없이 섞이고 9분위 −13.2 · 10분위 −26.1 이다.
#       **연속 온도계가 아니라 상위 20% 경보기로 읽어야 한다.**
#       게다가 이 Q4 효과는 2026-06 고점의 향후 90일 창(04-01~06-30, 61일)을 빼면
#       순위상관이 −0.311 → −0.005 로 사라진다. 사건 하나에 얹혀 있다는 뜻이라,
#       사이클이 하나 더 쌓이기 전에는 "낙폭을 예측한다"고 쓰지 말 것.
#   (2) 예전엔 "저온(25 미만)은 사실상 안 나온다"고 적혀 있었는데 지금은 4.6% 나온다.
#       그리고 그 저온 구간의 향후 90일 낙폭 중앙이 **−14.2%** 로, 초고온(−19.9%)과 거의
#       구분되지 않았다 — 지수가 "차갑다"고 말한 날 뒤에 폭락이 이어졌다. 2026-08-01
#       가중치 재보정(indicator_weights.py 참고)이 이걸 −3.4% 로 고쳤다.
#
# ※ 앵커 x값은 backtest/absolute.py 의 C안에서 **일괄 −1** 한 것이다(2026-08-06. 그 전엔
#   −2 였다). 백테스트의 원점수는 표본이 짧은 지표들을 관측 평균 **상수**로 놓아 추정한
#   값이라 실제보다 높게 나오는데, 그 편의만큼 x축을 내려 운영 원점수 눈금으로 옮긴다.
#
#   처음 −2 는 2026-07 에 **하루치(n=1)** 로 잰 −2.23(추정 34.15 vs 실측 31.92)이었다.
#   가중치가 검사 3(2026-08-01)에서 바뀌고 백테스트가 그걸 따라간 뒤 다시 쟀다(12거래일):
#
#       검사 2 가중치로 재현   +2.84 (중앙 +2.51)  ← 원래의 −2.23 이 재현된다
#       현행 가중치            +1.53 (중앙 +1.02)  ← 창을 19일로 넓히면 +0.88
#
#   즉 편의가 준 건 옛 측정이 틀려서가 아니라 백테스트가 현행 가중치를 쓰게 되면서
#   추정치가 내려갔기 때문이다. 뜯어보면 총편의 1.53 = 단기 상수 가정 1.24(std 1.76)
#   + 장기 축 눈금차 0.29(std 0.07)로, 흔들리는 몫은 전부 상수 가정에서 나온다.
#
#   ⚠️ **이 편의는 상수가 아니다.** 일자별로 −3.3~+5.9 까지 흔들린다 — 표본이 짧은 지표가
#   그날 자기 평균에서 얼마나 벗어났느냐에 딸려 있다. 스칼라 하나로 잡을 수 있는 양이
#   아니고, 표본 창도 12~19일로 짧으며 **전부 폭락 구간 안**이다. 근본 해결은 히스토리가
#   쌓인 뒤 상수 가정 자체를 걷어내는 것이고(2026-10 재보정 예정), 그때 이 보정은 없어진다.
#
...
SCORE_DISPLAY_ANCHORS = [
    (15.0, 0), (33.0, 31), (45.0, 50), (55.0, 72), (62.0, 86), (69.0, 96), (79.0, 100),
]
