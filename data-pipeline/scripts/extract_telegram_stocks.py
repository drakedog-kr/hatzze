...
# 가운뎃점으로 이어 붙인 나열("구독·플랫폼·디바이스")은 띄어쓴 수식어와 같은 자리다.
# 앞 글자가 한글도 영숫자도 아니라 경계 검사를 그냥 통과한다(modifier_context 참고).
MIDDLE_DOTS = "·‧・･"
...
def modifier_context(text: str, start: int) -> bool:
    ...
    가운뎃점 나열은 한 가지 예외다. `구독·플랫폼·디바이스` `CPU·스토리지·DB` 는 띄어쓴
    수식어와 똑같이 이름을 합성어 안에 넣는데, 앞 글자가 한글도 영숫자도 아니라 경계
    검사도 이 함수의 앞 낱말 검사도 그냥 통과한다. 저장된 언급 전량에서 이 자리를 세니
    12건이고 **전부 오탐이었다**(디바이스 6·DB 4·신흥 1·미래산업 1). 표본이 아니라
    전수라 판정이 갈릴 여지가 없어 함께 거른다. 쉼표 나열(`노바렉스, 디바이스`)은
    진짜 언급이 쓰는 자리라 건드리지 않는다.
    ...
    """
    if start > 0 and text[start - 1] in MIDDLE_DOTS:
        return True
