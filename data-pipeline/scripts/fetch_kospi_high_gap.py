...
# ⚠️ **화면에서는 "석 달"(차트 축만 "90일")로 말한다. "두 달"은 틀린 값이다.**
# 60거래일 ÷ 21거래일/월 = 2.86개월이라 석 달이 맞고, 두 달이면 42거래일이다.
# 한때 headline 은 석 달, 초보 설명은 두 달, 카드 축은 60일이라 같은 창을 세 가지로
# 부르고 있었다. 차트 축만 달력일(90일)을 쓰는 건 x축이 시간이라 그 단위가 자연스러워서다
# (실제 span 은 60거래일 ≈ 84일이라 90은 어림값이다).
# 정확한 거래일 수는 여기 상수와 코드 주석에만 둔다.
SPEED_WINDOW = 60
SPEED_META = {
    "slug": SPEED_SLUG,
    "name": "코스피 상승 속도",
    ...
    "category": "시장",
    "description_beginner": (
        "같은 고점이라도 천천히 오른 것과 석 달 만에 급하게 온 것은 다릅니다"
    ),
    "unit": "%",
}
