...
# 나눠 각각 한 문단으로 렌더한다.
# ⚠️ **이 블록은 주인공 문장과 추세 문장이 함께 쓴다.** 말투 예시를 고치면 두 문장이 같이
# 움직이므로 한쪽만 재고 넣지 말 것. [말투] 예시를 체언으로 맺지 않는 이유는 TREND_SYSTEM
# 위 주석에 있고, 그 변경이 주인공 문장에 미친 영향도 따로 쟀다(2026-07-30, 40회씩):
#
#     주인공  A 현행 비문 0/40 · 길이 준수 31/40    B 변경 비문 0/40 · 길이 준수 35/40
#
# 끝맺음 분포도 양쪽 다 흩어져 있어 회귀가 없다(보여줍니다 14↔13 · 나타냅니다 5↔8).
# 함께 찍힌 대체문자 5↔7 은 이 변경과 무관한 `콜�션` 건이다(fetch_put_call_ratio.py 주석).
COMMON = """\
당신은 한국 주식시장의 "과열도(온도)"를 보여주는 대시보드 '햇쩨(hatzze)'의 오늘의 요약을 쓰는 작성자입니다.
아래 데이터를 보고, 지시된 '한 문장'만 씁니다.

[말투]
- **모든 문장을 '~습니다'/'~ㅂ니다'로 끝맺습니다**(예: "~까지 식었습니다", "~로 보입니다", "~가 눈에 띕니다").
...
# 문단 3: 최근 추세 (②)
#
# **예시를 체언(명사)으로 맺지 않는다.** 예전엔 말투 예시와 아래 추세 예시가 둘 다
# "~한 흐름입니다"였는데, 그러면 모델이 문장을 명사로 맺으려 하다 계사('이')를 흘려
# 비문을 만들었다. 2026-07-23 에 "…오르내리는 흐름습니다"가 프로덕션에 나간 게 그것이고,
# common/text_check.py [2] 가 그때 생겼다.
#
# 실측(2026-07-30, 같은 digest 로 추세 문장 40회씩):
#
#     A 현행(흐름입니다 예시)      비문 2/40 · 앞 회차까지 7/80(8.75%) · 길이 준수 29/40
#     B 예시를 용언 종결로         비문 0/40                          · 길이 준수 33/40
#     C 계사 규칙을 덧붙임         비문 0/40                          · 길이 준수 25/40
#
# **비문만으로는 못 고른다**(A 대 B 는 Fisher 양측 p=0.094 로 표본이 모자라다). 갈린 건
# 다른 데였다 — A 는 40문장 중 34개가 똑같이 "~한 흐름입니다"로 끝났고(85%, 저장된 요약도
# 07-27~30 네 날 모두 그랬다) C 는 37개로 더 굳었다. B 는 표현이 흩어지고 길이 준수도
# 가장 좋다(올라왔습니다 20 · 올랐습니다 5 · 상승했습니다 4 · 올라섰습니다 3 · 회복했습니다 3).
#
# 고친 자리가 '흐름'이 아니라 **체언 종결이라는 구문**인 이유가 있다. A 에서 걸린 둘은
# `흐름습니다` 와 `상황습니다` 였다 — 특정 낱말을 정확히 보여줘도 모델은 다른 명사에서
# 같은 실수를 한다. 그래서 낱말을 고쳐 주는 것(#200 의 '콜옵션')으로는 안 되고, 명사로
# 맺는 본을 아예 안 보여주는 쪽이 맞다.
TREND_SYSTEM = COMMON + """

[이번 문장 — 최근 추세]
[최근 추세] 수치를 근거로, 최근 며칠간 '햇쩨 지수'가 어떻게 움직였는지 흐름을 한 문장에
담으세요. (예: "지난주 50℃대에서 며칠째 내려와 오늘 25℃까지 식었습니다.") 과거
