...
[이용약관](https://hatzze.fun/terms)과 [개인정보처리방침](https://hatzze.fun/privacy)은 베타 오픈일(2026-08-03)에 함께 시행됩니다. 두 문서는 나란히 놓고 읽히는 물건이라 조판을 한곳(`app/legal.tsx`)에서 정합니다.
...
LLM이 쓴 문장은 저장되기 전에 검수 레이어(`common/text_check.py`)를 지납니다. 깨진 글자·비문과, 모델이 토큰을 흘려 만든 없는 낱말을 걸러냅니다. 오늘의 요약·종목 내러티브·텔레그램 발송 글이 같은 그물을 씁니다.

AI가 쓴 글에는 **✨ 표시**가 붙습니다(히어로 요약·카더라 생태계 총평·종목 리포트 세 곳). 누르거나 마우스를 올리면 "이 요약은 생성형 AI가 작성했습니다."가 뜹니다. 아이콘만으로는 「인공지능 발전과 신뢰 기반 조성 등에 관한 기본법」 제31조가 요구하는 **표시 의무**를 채우지 못해서, 문구를 그 안에 담았습니다.

### 티커에 남의 시세를 싣지 않는 이유

예전엔 야후 파이낸스 시세 6종과 업비트 비트코인이었습니다. 걷어낸 이유는 둘입니다. **약관** — 야후는 자동 수집·상업적 이용·"경쟁하거나 실질적 대체물이 되는 데이터피드" 생성을 모두 금지하는데(ToS §2(e)(i)), 그 라우트가 정확히 야후 시세를 우리 도메인에서 최종 이용자에게 다시 내보내는 데이터피드였습니다. 업비트도 시세를 "회원 본인의 개인적 이용"에 한정합니다. 무료 실시간 시세는 사실상 전부 같은 제한이 걸려 있어 소스를 갈아끼워도 풀리지 않습니다. **그리고 시세 티커는 어디에나 있습니다** — 화면 맨 위 줄을 그걸로 채우면 정작 여기서만 볼 수 있는 것은 스크롤을 내려야 나옵니다.
...
- **하루치끼리 비교하지 않습니다.** 최근 3일 평균 vs **5일 이상 이전** 평균으로 봐야 메시지가 얇은 날의 요동이 가라앉습니다. 기준 창은 화면(`lib/telegram-data.ts`)과 발송 글(`common/broadcast_content.py`)이 같은 값을 씁니다 — 한쪽만 사이 이틀을 기준에 넣었더니 같은 날 같은 테마를 ▲3.7%p와 +5.6%p로 다르게 적었습니다.
- **"N개 채널"은 창 전체의 서로 다른 채널 수입니다**(`common/channel_breadth.py`). 일별 채널 수의 최댓값으로 세면 창을 넓힐수록 수가 줄어드는 일이 생깁니다 — 7일 창의 주간 결산이 3일 창의 사이트보다 작은 채널 수를 적었습니다.
- **유령 언급을 셈에서 뺍니다.** 종목명이 일반 단어나 다른 고유명사에 얹히면 없는 언급이 잡힙니다(회사명이 더 긴 낱말의 앞머리인 경우, 지주사 이름이 계열사 이름에 얹히는 경우). 경계 규칙과 오탐 위험군은 `config/stock_extraction.py`에 데이터로 두고 계속 키웁니다.

**동음이의는 목록이 아니라 자리로 가릅니다.** 상장 증권사 이름은 코퍼스 안에서 '리포트 발행처'와 '종목' 두 뜻으로 다 쓰입니다(`[SK증권 반도체]` vs `삼성증권 - 투자설명서`). 증권사를 통째로 제외하는 안은 못 씁니다 — 무작위 100건 중 진짜 종목 언급이 18건이었고 그중 13건이 DART 공시였습니다. 그래서 이름을 죽이지 않고 **표기의 자리**(대괄호 머리·괄호 귀속·줄 끝 귀속)만 발행처로 봅니다. 대상도 손으로 적지 않고 "이름이 '증권'으로 끝나는 상장사"로 잡아, 새 증권사가 상장해도 목록이 낡지 않습니다.

이름을 맞춰 세는 방식에는 천장이 있습니다. 겹치는 낱말을 가르는 건 규칙이 아니라 문장의 의미라서, 다음 단계는 이미 메시지를 읽고 있는 LLM에게 후보 종목을 함께 넘겨 확인받는 것입니다.
...
시세는 **야후 파이낸스 일봉**을 호출 시점에 직접 받아 계산합니다 — 별도 크론이나 테이블 없이 CDN에 15분 캐시합니다. 종가는 액면분할·감자를 소급 조정한 값이라 장기 낙폭이 왜곡되지 않습니다(`adjclose`는 감자에서 음수가 나와 쓰지 않습니다).
...
요청 시점에 바깥을 부르는 자리는 둘로 줄었습니다. **MDD 정밀분석**(`/api/mdd`)이 야후 일봉을 그 자리에서 받아 계산하고(종목을 고르는 화면이라 하루 한 번 갱신하는 배치에 얹을 수가 없습니다), **카더라 종목 카드**가 야후 현재가를 10분 캐시로 붙입니다(KRX 저장 종가는 며칠 지연이라 표시용으로 쓰면 어긋납니다. 실패하면 그 종가로 물러섭니다). 나머지 두 라우트는 바깥을 부르지 않습니다 — `/api/ticker`는 Supabase에 쌓인 카더라 집계를 읽어 탑바에 실어 보내고, `/api/channel-photo/[handle]`은 Supabase에 담긴 채널 프로필 사진을 **캐시되는 이미지 주소**로 돌려줍니다(HTML에 base64로 박아 보내면 같은 사진이 매 요청 페이로드에 실립니다).

**클릭하면 즉시 열립니다.** `loading.tsx` 자리표시자가 없으면 Next는 서버 페이로드가 다 올 때까지 전환을 커밋하지 않아, 눌러도 **이전 화면에 그대로 머뭅니다.** 카더라 리포트와 MDD 정밀분석 둘 다 자리표시자를 둡니다. 여기에 링크에 커서·손가락·포커스가 닿는 순간 시작하는 프리페치를 겹칩니다. 항상 켜 두지 않는 건, 그 페이지를 볼 생각이 없는 방문자까지 미리 받는 비용을 치르기 때문입니다.

**모바일(≤560px)은 구성이 다릅니다.** 사이드바가 숨고 내비게이션이 탑바 오른쪽 햄버거로 올라가며, 좁은 화면에서 읽히지도 않으면서 탑바를 다 차지하던 흐르는 티커는 걷어냅니다. 첫 방문에는 화면 아래에 PC를 권하는 안내가 한 번 뜹니다.
...
│  ├─ kadera/               #   카더라 리포트(텔레그램 분석) · loading.tsx = 전환 자리표시자
│  ├─ mdd/                  #   MDD 정밀분석(종목 낙폭·회복 통계) · loading.tsx
│  ├─ terms/                #   이용약관
│  ├─ privacy/              #   개인정보처리방침
│  ├─ legal.tsx             #   법정 고지 문서 두 장이 공유하는 조판
...
├─ githooks/                # 추적되는 커밋 훅(`git config core.hooksPath githooks`)
...
지표별 무게는 균등하지 않습니다. 1년치 실데이터(고점 3회·저점 3회)로 지표마다 **고점창 평균 − 저점창 평균 스프레드**를 재서 배정합니다. 방법론과 1차 근거는 [`docs/indicator-audit-2026-07-23.md`](docs/indicator-audit-2026-07-23.md)에 있고, **2026-08-01 전수 검사에서 25개를 전부 다시 쟀습니다**(근거는 `config/indicator_weights.py` docstring, 재현 스크립트는 `data-pipeline/backtest/`). 현재 가중치 합은 43.0이며, 가장 무거운 축은 거래대금 급증도(4.5)·신고가 괴리율(4.0)·초보 검색량(4.0)·코스피 상승 속도(3.0)입니다.

재보정에서 건진 것 둘:

- **정답지가 틀려 있었습니다.** 1차 검사는 2026-07-20(전고점 −28.5%)을 세 번째 저점으로 썼는데, 그건 문서를 쓰던 날 데이터의 끝이라 그렇게 보였을 뿐입니다. 실제 바닥은 07-30(−38.6%)이었고 07-20은 중간 반등점이었습니다. 저점 하나를 고치자 지표 순위가 눈에 띄게 바뀝니다(상승 속도 5위 → 2위).
- **스프레드만 보고 최적화하면 안 됩니다.** 가중치를 0~5로 훑으면 25개 중 13개가 "0이 최적"으로 나옵니다. 상위 몇 개가 신호를 다 들고 있어 나머지를 빼면 스프레드가 기계적으로 오르기 때문인데, 그대로 따르면 표본 기간의 58.8%가 초고온으로 찍히는(원점수 중앙값 82.5) 퇴화안이 됩니다.
...
- 고점권 외국인 매도 (지수가 52주 고점 −8% 이내일 때의 순매도만)
- 옵션 풋/콜 비율
...
`core.hooksPath` 설정 자체는 추적되지 않습니다. 커밋 훅이 안 도는 것 같으면 `git config --get core.hooksPath`부터 확인하세요(`githooks/README.md`).
...
**프리페치와 `loading.tsx`는 `npm run dev`로 확인할 수 없습니다.** 프리페치가 프로덕션 빌드에서만 동작해서, 전환 속도를 재려면 `npm run build:local` + `npm run start:local`로 띄워야 합니다.
...
발송 글에는 그물이 하나 더 걸립니다(`common/broadcast_content.py`). **채널에서 오간 말을 전하는 것과 그것을 권하는 것은 다르고, 막을 자리는 뒤쪽뿐입니다.** 그래서 시장에서 일어난 일을 서술하는 표현은 지나가고, 우리 문장이 조언으로 읽히는 자리만 좁게 잡습니다. 넓게 잡으면 사실을 옮긴 문장까지 걸려 글이 통째로 나가지 못합니다.
...
**"한국거래소 통계정보"는 글자 그대로 써야 하는 표기입니다.** KRX Open API 이용약관 제10조 ③이 "결과를 사용하여 화면을 제작한 경우, 해당 화면에 '한국거래소 통계정보'를 사용한 결과임을 명시"하도록 요구합니다. 푸터는 전 페이지 공통이라 그 한 줄로 모든 화면의 명시 의무가 채워집니다 — 모양을 맞추려고 "한국거래소(KRX)"로 되돌리면 약관이 지정한 문구가 아니게 됩니다.

**네이버 금융 스크래핑은 걷어냈습니다.** `finance.naver.com`의 robots.txt가 `User-agent: * / Disallow: /`라 애초에 긁으면 안 되는 곳이었습니다. 거기서 오던 지표 둘을 합법적인 소스로 바꿨는데, 값을 그대로 옮겨 담을 수는 없었습니다.

- **개인 순매수 강도 → 고점권 외국인 매도.** 개인 순매수 일별을 주는 무료 소스가 없습니다(ECOS 802Y001 일별엔 외국인만, 투자자별은 901Y055 월별뿐, KRX Open API엔 투자자별 서비스가 없음). 주체를 뒤집어 외국인을 쓰되, 그냥 갈아끼우면 코로나 폭락이 개인 광풍만큼 뜨겁게 찍힙니다 — 외국인은 폭락에도 팔고 고점에도 팔아서 froth의 비대칭이 없기 때문입니다. **지수가 52주 고점 −8% 이내일 때의 순매도만** 세어 "고점권인데 외국인이 판다 = 그 물량을 개인이 받고 있다"는 원래 논리를 되살렸습니다.
- **투자자예탁금 → 급등 종목 비율.** 개수가 아니라 비율인 게 핵심입니다. 상장 종목이 2,300개대에서 2,700개대로 늘기만 해도 개수는 오릅니다. 판정은 장중 고가가 아니라 **종가**입니다 — 상한가를 찍었다가 −22.75%로 마감한 종목까지 '급등'으로 세면 안 됩니다(2026-07-30 실측: 장중 터치 18 중 9가 풀렸습니다).
...
야후에서 받을 때 지키는 것: **확정된 종가만 저장합니다.** 오늘 칸에는 종가가 아니라 그 순간의 호가가 실려 오는데, 그대로 담으면 장중값이 종가로 굳습니다. 한 번 굳으면 빈 칸이 아니라 *틀린 값*이라 다음 실행이 덮어쓰지 않습니다. 그래서 오후 실행에 16:00 KST 하한을 두고, 시간봉 폴백은 이미 저장된 종가를 건드리지 않으며, 최근 며칠은 다시 확인합니다. 환율·금도 같은 규칙이고, 업비트 지표는 완결된 하루만 잽니다.

**MDD 정밀분석은 반대쪽입니다.** 저장하는 값이 아니라 지금 낙폭을 보여 주는 자리라 장중 시세를 그대로 씁니다. 그래서 "지금 이상 빠졌던 N번" 같은 표본 개수나 회복 카드의 중앙값 세로선이 장중에 오갈 수 있습니다. 고장이 아닙니다.
