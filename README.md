...
요청 시점에 바깥을 부르는 예외는 **상단 시세 티커**(`/api/ticker`)와 **MDD 정밀분석**(`/api/mdd`) 둘뿐입니다. 둘 다 배치가 아니라 그 자리에서 야후·업비트를 직접 호출하고 CDN에 짧게 캐시합니다 — 하루 한 번 갱신하는 배치에 얹기엔 성격이 다릅니다. 라우트가 하나 더 있는데(`/api/channel-photo/[handle]`), 이쪽은 외부 호출이 아니라 Supabase에 담긴 채널 프로필 사진을 **캐시되는 이미지 주소**로 돌려주는 자리입니다. HTML에 base64로 박아 보내면 같은 사진이 매 요청 페이로드에 실립니다.

**클릭하면 즉시 열립니다.** `loading.tsx` 자리표시자가 없으면 Next는 서버 페이로드가 다 올 때까지 전환을 커밋하지 않아, 눌러도 **이전 화면에 그대로 머뭅니다.** 여기에 링크에 커서·손가락·포커스가 닿는 순간 시작하는 프리페치를 겹칩니다. 항상 켜 두지 않는 건, 그 페이지를 볼 생각이 없는 방문자까지 미리 받는 비용을 치르기 때문입니다.

**모바일(≤560px)은 구성이 다릅니다.** 사이드바가 숨고 내비게이션이 탑바 오른쪽 햄버거로 올라가며, 좁은 화면에서 읽히지도 않으면서 탑바를 다 차지하던 시세 티커는 걷어냅니다. 첫 방문에는 화면 아래에 PC를 권하는 안내가 한 번 뜹니다.
