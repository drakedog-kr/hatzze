...
LLM이 쓴 문장은 저장되기 전에 검수 레이어(`common/text_check.py`)를 지납니다. 오늘의 요약·종목 내러티브·텔레그램 발송 글이 모두 같은 그물을 씁니다. 세 겹으로 봅니다 — ① 깨진 글자·고립 자모 ② 체언 뒤에 계사 없이 붙은 종결어미(비문) ③ 형태소 점수가 낮으면서 **원문에도 없는** 어절(`낙아들기` 같은 토큰 유실). ③은 점수만 걸면 고유명사가 무더기로 걸려서 원문 대조를 함께 봅니다.
...
- **유령 언급을 셈에서 뺍니다.** 종목명이 일반 단어와 겹치면 없는 언급이 잡힙니다(`하이브`←하이브리드가 언급을 2배로 부풀렸고, `미래산업`←하나증권 미래산업팀, 지주사 이름은 계열사 이름에 얹혀 세어졌습니다). 경계 규칙과 오탐 위험군 목록은 `config/stock_extraction.py`에 데이터로 두고 계속 키웁니다.

다만 **손으로 잇는 목록은 수렴하지 않습니다.** `미래산업팀`과 `미래산업(025560)`을 가르는 건 규칙이 아니라 문장의 의미입니다. 근본적인 수는 이미 메시지 본문을 읽고 있는 Haiku에게 후보 종목을 함께 넘겨 확인받는 것인데, 그 스크립트가 감성 지표의 유일한 공급원이라 프롬프트를 건드리면 톤이 밀렸는지 보이기가 어렵습니다(같은 입력 2,987건에 낙관도가 36%↔48%로 흔들립니다). 잴 때는 집계 %가 아니라 같은 메시지를 짝지은 **라벨 뒤집힘률**로 봐야 합니다.
...
요청 시점에 바깥을 부르는 예외는 **상단 시세 티커**(`/api/ticker`)와 **MDD 정밀분석**(`/api/mdd`) 둘뿐입니다. 둘 다 배치가 아니라 그 자리에서 야후·업비트를 직접 호출하고 CDN에 짧게 캐시합니다 — 하루 한 번 갱신하는 배치에 얹기엔 성격이 다릅니다. 라우트가 하나 더 있는데(`/api/channel-photo/[handle]`), 이쪽은 외부 호출이 아니라 Supabase에 담긴 채널 프로필 사진을 **캐시되는 이미지 주소**로 돌려주는 자리입니다. 예전엔 base64로 HTML에 박혀 2.5MB가 매 요청 렌더를 막았습니다.

**클릭하면 즉시 열립니다.** 화면 전환의 병목은 서버 시간이 아니라 *기다림이 눈에 보이는지*였습니다. `loading.tsx` 자리표시자가 없으면 Next는 서버 페이로드가 다 올 때까지 전환을 커밋하지 않아, 눌러도 **이전 화면에 그대로 머뭅니다**(/kadera는 데이터 가지가 12개라 그게 0.7~1.5초였습니다). 여기에 링크에 커서·손가락·포커스가 닿는 순간 시작하는 프리페치를 겹칩니다. 항상 켜 두지 않는 건, 카더라를 누를 생각이 없는 방문자까지 전원이 5분마다 풀 렌더를 서버에 시키기 때문입니다 — **의도를 보인 사람만 비용을 냅니다.**

**모바일(≤560px)은 구성이 다릅니다.** 사이드바가 숨고 내비게이션이 탑바 오른쪽 햄버거로 올라가며, 흐르는 시세 티커는 걷어냅니다(좁은 화면에서 읽히지도 않으면서 탑바를 다 차지했습니다). 첫 방문에는 화면 아래에 "지표를 한눈에 보시려면 PC를 권해 드립니다" 토스트가 한 번 뜹니다. "최적화되지 않았습니다" 류의 문구를 피한 건, 모바일이 고장난 게 아니라 카드를 한눈에 견주기 어려울 뿐인데 첫 화면에서 사과부터 읽으면 남은 화면까지 의심하게 되기 때문입니다.
...
│  └─ api/                  #   ticker(실시간 시세) · mdd(낙폭 계산) · channel-photo(채널 아바타)
...
발송 글에는 금지어 그물이 하나 더 걸립니다(`common/broadcast_content.py`). 다만 **다섯 개뿐입니다** — 매매 신호 · 매수 의견 · 매도 의견 · 매수 추천 · 매도 추천. 한때 54개였는데, 시황을 서술하는 말(강세·반등·신고가·목표주가…)까지 막다가 "가이던스 상향이 잇따랐습니다" 같은 사실 보도가 걸려 아침 글이 통째로 안 나간 날이 있습니다. **채널에서 오간 말을 전하는 것과 그것을 권하는 것은 다르고, 막을 자리는 뒤쪽뿐입니다.** 낱말이 아니라 조합으로 잡는 것도 같은 이유입니다. '매수'를 통째로 막으면 "외국인 순매수"가 죽지만 '매수 추천'은 권유 말고 다른 뜻이 없습니다.
...
야후에서 받을 때 지키는 것: **확정된 종가만 저장합니다.** 오늘 바에는 장중 호가가 실려 있어서 그대로 담으면 장중값이 종가로 굳고, 한 번 굳으면 빈 칸이 아니라 *틀린 값*이라 다음 실행이 덮어쓰지 않습니다. 그래서 오후 실행에 16:00 KST 하한을 두고(실측: 마감 +23분에 확정), 시간봉 폴백은 이미 저장된 종가를 건드리지 않으며, 최근 며칠은 다시 확인합니다. 환율·금도 같은 규칙이고, 업비트 지표는 완결된 하루만 잽니다.

**MDD 정밀분석은 반대쪽을 골랐습니다.** 야후 일봉의 오늘 칸에는 종가가 아니라 그 순간의 호가가 실려 오는데, 이 화면은 저장하는 값이 아니라 지금 낙폭을 보여 주는 자리라 그대로 씁니다. 그래서 "지금 이상 빠졌던 N번" 같은 표본 개수나 회복 카드의 중앙값 세로선이 장중에 오갈 수 있습니다. 고장이 아닙니다.
