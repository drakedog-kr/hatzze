...
지금 시장이 어떤 상태인지를 네 화면으로 보여주는 대시보드입니다.

- **시장 브리핑** (`/`) — 시장·감성 지표 **25개**를 매일 종합한 하나의 **온도**(℃)
- **국장 카더라** (`/kadera`) — 주식 텔레그램에서 국내 종목이 어떻게 회자되는지
- **미장 카더라** (`/kadera/us`) — 같은 채널들이 **미국 종목**은 뭐라고 하는지
- **MDD 정밀분석** (`/mdd`) — 내 종목이 고점에서 얼마나 내려왔고, 과거엔 얼마 만에 돌아왔는지

다섯 번째 화면 **서학개미 해부도**는 준비 중입니다. 사이드바에 예고 항목으로만 걸어 두고, 링크가 아니라 누를 수 없는 자리로 뒀습니다.

**2026-08-06 베타 오픈**, **2026-08-12 미장 카더라 추가**. 서비스 전체가 베타라 로고 옆에 배지를 답니다. 지표는 계속 늘고 눈금도 다시 잡히는 중이라, 시장 브리핑 맨 아래에 새 지표를 제보받는 자리를 뒀습니다.
...
수집은 **Telethon**(Telegram Client API), 종목 추출은 **KRX 상장 2,700여 종목 사전 + 별칭 매칭**, 메시지 톤·화제어 분류와 총평 작성은 **Claude Haiku**가 맡습니다. 테마 로테이션은 **26테마 사전**을 씁니다. 모니터링 채널 목록은 레포에 두지 않고 Supabase에서 런타임 조회합니다.
...
**같은 채널들**이 미국 종목은 뭐라고 하는지 봅니다. 새로 채널을 늘린 게 아니라, 이미 수집하고 있던 국내 채널의 메시지에서 **미국 종목 이야기만 따로 뽑아낸** 화면입니다. 최근 30일 안에 미국 종목을 한 번이라도 언급한 채널이 대상인데, 제목에 '미국'이 없는 평범한 국내 채널이 대부분입니다.

| | 카드 | 보여주는 것 |
|---|---|---|
| **히어로** | 미국 얘기 채널 | 미국 종목을 말하는 채널 규모 |
| | 생태계 센티먼트 | 미국 종목 메시지의 톤 |
| | **오늘의 요약** | 그날 오간 미국 이야기를 **Claude Haiku**가 |
| **최근 뜨는 것** | 급부상 종목 | 평소보다 언급이 갑자기 뛴 미국 종목 |
| | 테마 로테이션 | 관심이 어느 미국 테마로 옮겨가는지 (**16테마·178티커** 사전) |
| | 이슈 키워드 | 종목명이 아닌 화제어 |
| **무슨 얘기가 오갔나** | 주요 종목 리포트 | 가장 많이 회자된 미국 종목의 추이와 흐름 |
| | 트렌딩 메시지 | 미국 종목을 말한 글 중 가장 널리 퍼진 것 |
| | 미국×국내 동시 언급 | 한 메시지에서 함께 불린 미국·국내 종목 쌍 |

티커 매칭은 국내 종목보다 까다롭습니다. 세 글자 대문자가 일상어·국내 종목명과 겹치기 때문입니다.

- **`$` 접두는 미국 신호가 아닙니다.** 이 코퍼스는 국내 종목에도 붙입니다(`$NAVER $카카오`). 그래서 기호가 아니라 **메시지 전체의 단서**로 가릅니다.
- **국내 종목명과 충돌하는 티커**는 사전에서 뺍니다. 국내에 같은 이름의 회사가 있으면 그쪽 언급이 미국 종목으로 잡히기 때문입니다.
...
flowchart LR
    ...
    T1["fetch_telegram.py<br/>Telethon · 채널 메시지"] --> T2["국내 종목 추출 · 일별 집계<br/>종목 · 테마"]
    T1 --> U1["미국 티커 추출 · 일별 집계<br/>종목 · 테마 · 동시언급"]
    ...
    G["daily-update.yml<br/>매일 06:30 · 17:30 KST 발화"] -.->|트리거| A
    G -.->|트리거| T1
...
한 실행 안에서는 **카더라가 먼저, 지표가 나중**입니다. 카더라는 KRX 와 무관해 아무 때나 돌 수 있고, 지표는 KRX 가 전 영업일 자료를 올리는 08:00 KST 를 기다려야 하기 때문입니다. 덕분에 카더라 면이 지표보다 한 시간쯤 먼저 새로워집니다.
...
**네 화면이 같은 시각 언어를 씁니다.** 카드를 낱장으로 흩지 않고 한 판의 시트에 담고, 온도는 50을 경계로 파랑(차가움)↔빨강(뜨거움) 두 색으로만 말합니다. 색·간격·모서리는 전역 토큰(`app/globals.css`·`app/ui.tsx`) 한 벌에서 나오므로 한 페이지만 갈라지지 않습니다.
...
│  ├─ kadera/               #   국장 카더라(텔레그램 분석) · kadera/us(미장)
...
│  └─ common/               #   Supabase·야후·KRX·HTTP 클라이언트 · 발송 문안 · LLM 문장 검수
├─ supabase/                # 스키마 + 마이그레이션 SQL(migration_001~041)
├─ docs/                    # 지표 감사 기록
...
한 실행은 **카더라 → 지표** 순입니다. 카더라 파이프라인(채널 동기화 → 메시지 수집 → 커버리지 검사 → LLM 분류 제출 → 영향력 점수 → 국내·미국 종목 추출 → 종목·테마 집계 → 분류 수거 → 센티먼트 → 총평 생성)이 먼저 돌고, KRX 공표 대기 게이트를 지난 뒤 지표 수집·점수 계산이 이어집니다.

각 스텝은 `continue-on-error`로 개별 실패가 전체를 막지 않으며, 실패가 있으면 알림 이슈를 열어 추적합니다. 알림 이슈는 **제목이 곧 진단**이 되도록 갈래를 나눠 뒀습니다 — 데일리 파이프라인 실패(지표) · 카더라 수집 중단 · 카더라 수집 커버리지 저하 · 국장 카더라 집계 중단 · 카더라 LLM 분류 중단 · 미장 카더라 집계 중단. 한 배열에 몰아넣으면 어디가 멈췄는지 이슈를 열어 봐야 압니다. **조용한 고장**을 막는 감시도 둘 걸어 뒀습니다.
...
`supabase/schema.sql`을 Supabase SQL Editor에 붙여넣어 실행하면 `indicators`, `indicator_values`, `daily_score` 3개 테이블과 RLS(공개 읽기 전용·쓰기는 service_role)가 설정됩니다. 이후 스키마 변경은 `supabase/migration_*.sql` 파일로 관리합니다(현재 041까지).

여기에 KRX 상장종목 마스터 `stocks`(MDD 검색창·종목 추출이 함께 씁니다)와 카더라용 `telegram_*` 테이블(국장·미장 양쪽)이 마이그레이션으로 붙습니다. `telegram_*`는 공개 읽기를 열지 않아서, 프론트도 `SUPABASE_SECRET_KEY`로 조회합니다.
