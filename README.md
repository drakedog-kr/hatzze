...
**시계를 깃헙 밖으로 옮긴 이유.** 깃헙 예약은 부하가 높으면 지연되고 심하면 슬롯을 통째로 버립니다(공식 문서). 2026-08-26~28에 실행이 3~10시간씩 늦게 생기다가, 08-28에는 네 발화 중 둘이 8시간 늦게 오고 둘은 아예 오지 않았습니다. `workflow_dispatch`는 배치 스케줄러가 아니라 실시간 경로라 생성과 시작이 같은 초입니다.
