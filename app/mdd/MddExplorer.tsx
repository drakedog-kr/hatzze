...
function RiskProfile({ r, periodLabel }: { r: RiskProfileData; periodLabel: string }) {
  ...
  /* 타일 제목 옆 괄호. 두 타일과 한 타일이 서로 다른 것을 적는데, 다르게 적을 이유가 있다.
     ...
     적는다. 막대는 최근 5줄이지만 나머지 해는 전체보기 안에 있다.

     ⚠️ 이 타일에서 전체보기를 걷어내면 괄호도 같이 손봐야 한다. 그러면 10년이라 적고 5년만
     보여주는 상태로 되돌아간다(2026-07-30 에 실제로 그래서 어색했다).

     ⚠️ 이 괄호는 요약의 분모이기도 하다. 요약이 "6년은 수익이 더 컸습니다"라고만 적고
     '10년 중'을 생략하는데(자리가 없다), 그 10년을 말해 주는 게 여기와 전체보기 둘뿐이다.
     괄호를 지우면 6이 무엇 중 6인지 화면 어디에도 남지 않는다. */
  ...
  /* 큰 수치와 요약은 **조회 기간 전체**(scoped)로 센다. 막대에 깔린 최근 다섯 해가 아니다.
     ...
     10년 중 6년.) 2026-08-04.
     ⚠️ 여기 적힌 '보상 배수'는 지금 화면에 없다. 읽히지 않아 걷어냈다(아래 foot 주석).
     창을 맞춰야 하는 이유는 그대로다 — 평균 낙폭과 이긴 해 수가 여전히 scoped 를 센다. */
    ...
    yearly.length === 0
      ...
      : {
          ...
          /* 여기만 fmtPct 를 안 쓴다. 그 함수는 부호(+/−)를 붙이는데, 뒤에 '수익/손실'을
             ...
             읽히니 낱말을 남기고 부호를 뺀다. */
          /* 비율("보상 0.89배")을 걷어내고 두 수치를 그대로 말한다.

             '배'는 사람들이 원금 배수를 말할 때 쓰는 낱말이라, 1 미만이면 **손해로 먼저
             읽힌다.** 삼성전자 0.89배가 실제로 그렇게 읽혔다 — 10년간 7.9배로 늘어난
             종목인데 "0.89배면 손해 아닌가"가 첫 반응이었다. 앞에 "복리로는"을 붙여
             기준을 밝혀도 숫자 생김새가 이기므로, 비율이라는 형태 자체를 버렸다.

             옆 절과 기준이 갈리는 문제도 같이 풀린다. 0.89 는 복리(CAGR)로 낸 값인데
             "6년은 수익이 컸다"는 해마다의 단순 수익이라, 한 문장 안에서 잣대가 둘이었다.
             변동이 크면 복리가 산술평균보다 낮아 둘이 어긋난다(삼성전자 10년: 산술 29.7%
             vs CAGR 22.9%, 비율로는 1.15 와 0.89). 수치를 그대로 적으면 독자가 둘을
             나눌 일이 없어 어긋남이 드러나지 않는다.

             수익은 머리 수치와 같은 소수 한 자리로 적는다. 반올림해 "23%"로 적으면 두 줄
             위 "22.9%"와 어긋나 같은 상자에서 숫자가 둘이 된다.

             '10년 중'은 자리가 없어 뺐다. 각주 예산이 48자인데 넣으면 51자가 되고, 창은
             타일 제목 괄호(yearScope)와 전체보기 버튼이 이미 말한다.

             beatCount 가 0 이면 "0년은 수익이 더 컸습니다"가 되어 읽히지 않으므로 갈래를
             나눈다. 연평균이 플러스인데도 이긴 해가 없는 조합은 실제로 가능하다 — 한 해가
             크게 벌어 복리를 끌어올리고 나머지 해는 전부 낙폭에 졌을 때다. */
          foot: summary(
            r.annualReturn >= 0 && avgYearMdd < 0
              ? `해마다 ${Math.abs(Math.round(avgYearMdd))}% 낙폭을 견디고 연 ${r.annualReturn.toFixed(1)}%를 벌었습니다 · ${beatCount > 0 ? `${beatCount}년은 수익이 더 컸습니다` : "낙폭이 늘 더 컸습니다"}`
