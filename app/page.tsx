    ...
    <Shell hit={v.isHit} minH={210}>
      ...
      <div style={{ margin: "6px 0 2px" }}>
        ...
        {/* 게이트 상태. 이 카드에서 가장 자주 받는 질문이 "왜 과열도가 0인가" 일 텐데,
            ...
            배지 하나로 끝낸다. 옆에 "52주 고점 대비 -38.6%" 를 같이 적었는데, 배지가
            이미 답(고점권이냐 아니냐)을 말한 뒤라 숫자는 근거를 한 번 더 되풀이할 뿐이고,
            같은 화면 위쪽의 신고가 괴리율 카드가 이미 그 수치를 제 주인공으로 크게 쓴다.

            ⚠️ 조건은 여전히 gap 을 본다. 화면에 안 적어도 이게 "고점권 판정에 필요한
            자료가 이 행에 있느냐"의 시험이기 때문이다 — 옛 행(개인 순매수 시절)에는
            gap 도 at_high 도 없어서, 조건을 빼면 그 행들이 근거 없이 "고점권 아님" 을
            단언하게 된다. atHigh 는 값이 없으면 조용히 false 라 혼자서는 못 가른다. */}
        {gap != null && (
