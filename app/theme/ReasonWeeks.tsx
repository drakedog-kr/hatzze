...
import { themeStockHref, type ThemeMarketKey } from "@/lib/theme-href";
 ...
 * 아니다 — 월요일에 열면 이번 주가 하루뿐이라 비어 보인다. 단추 이름이 '지난주'였을 땐 달력 주로 읽혔다(2026-09-22).
 *
...
export function ReasonWeeks({
  ...
  /** 국장·미장 — 종목 링크의 꼴을 가른다(lib/theme-href.ts themeStockHref). 함수는 서버에서 못 넘기니 키만 받는다. */
  market: ThemeMarketKey;
  /** 등락률이 무엇인지(툴팁). 국장은 그날 종가, 미장은 채널이 적은 값(app/theme/market.ts). */
  rateNote: string;
  rows: ThemeReasonRow[];
  latest: string;
  earliest: string;
  ...
  // 이 주의 줄을 날짜로 묶는다(오래된 날이 위, 최신 날이 아래 — 달력 순서. 최신을 위에 두니 거꾸로 읽혀 헷갈렸다, 2026-09-22.
  // 같은 날은 여러 채널이 말한 것이 먼저). 기본 꼴은 **날짜 머리 하나 아래 그날 종목들**이다
  // (2026-09-21). 두 열 격자(선이 층계)와 날짜 칸을 둔 한 표(같은 날짜가 세 줄) 둘 다 이보다 못했다.
  const days = useMemo(() => {
  ...
  return (
    <div>
      {/* 머리(SectionHead)를 여기서 그린다 — 오른쪽 알약이 보고 있는 주의 날짜라 주를 넘길 때 같이 바뀌어야 한다
          (TrendingTabs 가 기간 탭 때문에 머리를 안에서 그리는 것과 같은 사정). 총 건수는 안 적는다(2026-09-21). */}
      <SectionHead
        icon="history"
        title="등락의 이유"
            ...
            <section key={date} className="hz-reason-day" aria-label={fmtKoWd(date)}>
              ...
              {list.map((r) => (
                <div key={`${date}-${r.code}`} className="hz-trow hz-cols-theme-reason">
                  <Link href={themeStockHref(market, r.code)} style={{ display: "flex", alignItems: "center", gap: 9, minWidth: 0, textDecoration: "none" }}>
                    ...
                    <span className="hz-theme-namerow">
                      ...
                      {r.changeRate == null ? (
                        /* 미장은 '종가 전'이 없다 — 채널이 등락을 안 적은 줄이라 그냥 "—". */
                        <span style={{ marginLeft: 8, fontSize: "var(--fs-11)", color: C.muted, whiteSpace: "nowrap" }}>{market === "kr" && date === newestDate ? "종가 전" : "—"}</span>
                      ) : (
                        <span
                          title={rateNote}
                          style={{
      ...
      {/* 카더라 달력의 날짜 이동 단추와 같은 알약. 갈 데가 없는 쪽은 그리지 않는다 — 흐린 '다음 7일'이 남아 있으면
          없는 것이 있는 듯 읽힌다(2026-09-22). 사이의 빈 칸(flex:1)이 자리를 지켜 남은 단추는 제 쪽에 선다. */}
