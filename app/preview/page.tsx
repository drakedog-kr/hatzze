    ...
    <>
      ...
      <section className="hz-sheet">
        ...
        {wall.length === 0 ? (
          ...
          <>
            {/* ⚠️ **바닥 각주 띠(.hz-sheet-foot)를 다시 달지 말 것**(2026-09-03 결정).
                한때 "개장·장 중·종가 모두 최근 5년 평균입니다" 가 거기 있었는데, 그 한 줄
                때문에 시트 바닥에 띠를 한 칸 더 두는 게 아깝다는 판단이었다. 붙일 만한 말을
                열 개 뽑아 봤지만(표본 크기·대조군·사전 규모·개장 비중) 어느 것도 띠 한 칸
                값을 하지 못했다.
                ⭐ **그 사실은 이미 타일 안에 있다** — 줄 목록 머리의 "최근 5년, 이렇게 오른 날
                국내 개장은" 이 타일마다 같은 말을 한다. 각주는 그걸 열세 번째로 되풀이했다.
                ⚠️ '매수·매도 신호가 아니다' 고지는 전역 푸터가 한다(화면 머리 주석 참고). */}
