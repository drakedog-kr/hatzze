    ...
    <>
      ...
      {ch.turnover && (
        /* ⚠️⚠️ 알약이 `최근 10년` 이었다. 그건 아래 표가 덮는 기간이지 **이 숫자가 언제
            ...
            ⛔ 물음표(`noteHelp`)를 뗐다. "거래액은 실측이고 나누는 잔고는 추정이라
            ±20%면 5.9~8.8개월입니다" 라고 적었는데 **읽어도 뜻이 안 잡힌다**는 말을
            들었다 — '나누는 잔고' 가 무엇인지 화면 어디에도 없으니 당연하다.
            ⚠️ 그래서 **분모가 추정이라는 사실이 화면에서 사라졌다.** 결론 문장의
            '쯤' 하나가 그걸 지고 있다. 되살린다면 툴팁이 아니라 카드 안에 폭을
            그리는 쪽이어야 한다(5.9~8.8 을 막대의 옅은 띠로). */
