...
const BADGE_FONT = 8;
/**
 * 배지 안 글자의 행간(px). **지름과 일부러 다르다.**
 *
 * 이 값이 하는 일은 베이스라인을 놓는 것 하나다. 행간이 L 이면 반-리딩이 (L − 10)/2 이고
 * (8px Pretendard 의 폰트 상자가 위 8 · 아래 2 다) 베이스라인은 거기에 8 을 더한 자리다.
 * 10.7 이면 베이스라인이 동그라미 위에서 **8.35px** 이 된다.
 *
 * ⭐ 8.35 는 계산이 아니라 **찍어 보고 고른 값**이다. 같은 래스터라이저로 베이스라인을
 * 0.1 씩 옮겨 가며 잉크의 알파 무게중심이 동그라미 가운데(5.5)에서 얼마나 벗어나는지
 * 쟀다. 글자가 화소 격자에 스냅되느라 값이 층계로 움직이고, 그 층계가 배율마다 다르다.
 *
 *   베이스라인   dpr 1     dpr 2     dpr 3      (+ 는 아래로 처짐, px)
 *   8.2         −0.365    −0.497    −0.041
 *   8.3~8.4     −0.365    +0.003    −0.041   ← 셋을 함께 보면 여기가 가장 낫다
 *   8.5~8.7     +0.635    +0.003    +0.292   ← 행간을 지름과 같게 두면 여기였다
 *
 * ⚠️ 행간을 지름과 같게 두면 **레티나에서만 가운데다.** dpr 2 는 0.003 인데 일반 해상도
 * (dpr 1)에서 0.635px 이 처진다. 재보기 전에는 이걸 못 본다 — dpr 1 짜리 캔버스로 재야
 * 드러난다. 8.3~8.4 의 한가운데인 8.35 로 옮기면 dpr 2 는 그대로고 dpr 3 은 0.292 → 0.041,
 * dpr 1 은 0.635 → 0.365 로 준다. 세 배율 어디서도 나빠지지 않는다.
 * 살짝 위로 가는 쪽을 고른 것이기도 하다 — 동그라미 안의 한 글자는 기하학적 정중앙보다
 * 아주 조금 위에 있을 때 가운데로 읽힌다.
 *
 * ⚠️ 이 값은 **글꼴 메트릭에 매여 있다.** 서체나 글자 크기를 바꾸면 위 표를 다시 재야 한다.
 */
const BADGE_LINE_HEIGHT = 10.7;
/**
    ...
    <span
      ...
      style={{
        ...
        /* ⚠️⚠️ **flex 가운데 정렬(alignItems:center)로 앉히지 말 것.** 그건 글자의 잉크가
           ...
           대신 행간으로 베이스라인을 직접 놓는다 — 얼마에 놓을지는 BADGE_LINE_HEIGHT 주석에
           실측 표가 있다. 가로는 text-align 이 글자 **폭**을 가운데 두는데, N 은 좌우 side
           bearing 이 0.43 으로 같아서 잉크까지 그대로 가운데에 온다(실측 어긋남 0.06px). */
        display: "block",
        width: BADGE_SIZE,
        height: BADGE_SIZE,
