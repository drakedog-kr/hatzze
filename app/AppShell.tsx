...
/**
 * 테마 리포트의 국장·미장 서브 아이콘 — **태극기의 건곤감리**와 **엠파이어 스테이트 빌딩 상층부**(2026-09-22, 참고 이미지 둘을 받아 그렸다).
 * 카더라 서브(태극·여신상)와 같은 '나라 상징' 결이지만 그림이 다르다. 그 전 시안(원화·달러 · 지도 · 무궁화/별 · 한옥/돔 ·
 * 글자 타일 · 해/달 · 국기 자리의 건곤감리)은 저장소 밖 시안 페이지(hatzze-icons 폴더)에서 18px 로 견줘 보고 접었다.
 *
 * ## 건곤감리 — 마름모
 *
 * 네 괘가 **마름모의 네 변**에 선다(참고 이미지의 배치). 건(☰) 왼쪽 위 · 감(☵) 오른쪽 위 · 곤(☷) 오른쪽 아래 · 리(☲) 왼쪽 아래 —
 * 태극기와 같은 자리다. 막대 끝은 가로·세로 축을 따라 잘려 사다리꼴이고 안쪽 막대일수록 짧다. **위 둘은 테두리만, 아래 둘은
 * 채운다** — 국장 카더라의 태극이 파랑(아래)만 채우는 규칙 그대로(색이 하나뿐이라 빨강·파랑을 이렇게 가른다).
 * 비율은 참고 이미지 실측(막대 22 · 틈 12 · 세 막대 깊이가 변 거리의 67%)이다. 깊이를 더 키우면 넷이 동심 마름모 고리로 보이고,
 * 무리 사이 틈(GX)이 막대 틈과 같아도 그렇다. 좌표는 생성 스크립트(gen.py, 저장소 밖 hatzze-icons 폴더)가 만들었다 — 손으로 고치지 말고 거기서 다시 뽑을 것.
 *
 * ## 엠파이어 스테이트 — 관(冠)만
 *
 * 참고 이미지처럼 상층부만: 안테나 · 고리 둘 · 좁아지는 첨탑 · 관 기단 두 단 · 윗단 · 몸통과 양옆 두 단의 날개, 아래는 잘라 낸다.
 * 참고는 선 그림인데 18px 에서 선은 안 남아 **실루엣**으로 옮겼다(날개 사이 0.4 틈이 큰 크기에서 층을 만든다).
 * 비율은 참고 실측(높이 570 기준 안테나 25% · 첨탑 21%).
 *
 * 둘 다 여신상과 같은 BLEED 장치로 1.2배 크게 그린다(LibertyIcon 주석의 이유) — 잉크가 얇아 같은 상자에 그리면 옆 아이콘보다 작아 보인다.
 */
...
function KrTrigramsIcon({ size = 20 }: { size?: number }) {
  ...
  return (
    <svg width={draw} height={draw} viewBox="0 0 24 24" aria-hidden="true" style={{ display: "block", flexShrink: 0, margin: -pull }}>
      {/* 건 ☰ · 왼쪽 위 · 테두리만 */}
      <path d="M0.92 11.20 L11.20 0.92 L11.20 3.04 L3.04 11.20 Z M4.03 11.20 L11.20 4.03 L11.20 6.15 L6.15 11.20 Z M7.14 11.20 L11.20 7.14 L11.20 9.26 L9.26 11.20 Z" fill="none" stroke="currentColor" strokeWidth="0.9" />
      {/* 감 ☵ · 오른쪽 위 · 테두리만 */}
      <path d="M12.80 0.92 L17.69 5.81 L16.63 6.87 L12.80 3.04 Z M18.19 6.31 L23.08 11.20 L20.96 11.20 L17.13 7.37 Z M12.80 4.03 L19.97 11.20 L17.85 11.20 L12.80 6.15 Z M12.80 7.14 L14.58 8.92 L13.52 9.98 L12.80 9.26 Z M15.08 9.42 L16.86 11.20 L14.74 11.20 L14.02 10.48 Z" fill="none" stroke="currentColor" strokeWidth="0.9" />
      {/* 곤 ☷ · 오른쪽 아래 · 채움 */}
      <path d="M23.08 12.80 L18.19 17.69 L17.13 16.63 L20.96 12.80 Z M17.69 18.19 L12.80 23.08 L12.80 20.96 L16.63 17.13 Z M19.97 12.80 L16.63 16.14 L15.57 15.08 L17.85 12.80 Z M16.14 16.63 L12.80 19.97 L12.80 17.85 L15.08 15.57 Z M16.86 12.80 L15.08 14.58 L14.02 13.52 L14.74 12.80 Z M14.58 15.08 L12.80 16.86 L12.80 14.74 L13.52 14.02 Z" fill="currentColor" />
      {/* 리 ☲ · 왼쪽 아래 · 채움 */}
      <path d="M11.20 23.08 L0.92 12.80 L3.04 12.80 L11.20 20.96 Z M11.20 19.97 L7.86 16.63 L8.92 15.57 L11.20 17.85 Z M7.37 16.14 L4.03 12.80 L6.15 12.80 L8.43 15.08 Z M11.20 16.86 L7.14 12.80 L9.26 12.80 L11.20 14.74 Z" fill="currentColor" />
    </svg>
  );
}
...
function UsEmpireIcon({ size = 20 }: { size?: number }) {
  ...
  return (
    <svg width={draw} height={draw} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" style={{ display: "block", flexShrink: 0, margin: -pull }}>
      <path d="M11.50 0.30 L12.50 0.30 L12.50 6.00 L11.50 6.00 Z M10.90 6.00 L13.10 6.00 L13.10 6.70 L10.90 6.70 Z M10.60 6.90 L13.40 6.90 L13.40 7.60 L10.60 7.60 Z M10.80 7.60 L13.20 7.60 L14.00 12.40 L10.00 12.40 Z M9.40 12.40 L14.60 12.40 L14.60 13.30 L9.40 13.30 Z M8.90 13.50 L15.10 13.50 L15.10 14.40 L8.90 14.40 Z M8.60 14.60 L15.40 14.60 L15.40 16.40 L8.60 16.40 Z M9.60 16.40 L14.40 16.40 L14.40 23.60 L9.60 23.60 Z M6.90 17.40 L9.20 17.40 L9.20 23.60 L6.90 23.60 Z M14.80 17.40 L17.10 17.40 L17.10 23.60 L14.80 23.60 Z M4.40 19.60 L6.50 19.60 L6.50 23.60 L4.40 23.60 Z M17.50 19.60 L19.60 19.60 L19.60 23.60 L17.50 23.60 Z" />
    </svg>
  );
}
