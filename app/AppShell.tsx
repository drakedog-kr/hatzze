...
/**
 * 내부자 아이콘. 중절모 + 선글라스 + 코트 어깨의 실루엣이다.
 *
 * Material Symbols 를 먼저 뒤졌다: 이 그림에 해당하는 글리프가 없다. contact_page ·
 * assignment_ind · badge 는 전부 "문서에 붙은 사람"이라 신분증으로 읽히고, 이 화면이
 * 말하려는 "얼굴을 감춘 쪽의 기록"이 안 나온다.
 *
 * ⭐ 손으로 그리지 않고 **원본 이미지를 potrace 로 땄다.** 손으로 그렸을 때 선글라스
 * 모양이 어긋났고(알이 붙은 띠로 그렸는데 원본은 콧등에서 갈린다), 코트 라펠은 아예
 * 빠져 있었다. 실루엣은 눈으로 재현하면 반드시 틀린다.
 *
 * 딴 방법(다시 뽑아야 하면 그대로 재현할 것):
 *   흰 픽셀(>128)의 경계상자로 자르고 → 4배 확대 + 가우시안 0.6 (JPEG 경계 잡티 제거)
 *   → 문턱 128 → potrace(turdsize=64, alphamax=1.0, opticurve, opttolerance=0.8)
 *   → 잉크가 20.5/24 가 되도록 정규화. 세그먼트 48개.
 *
 * ⭐ 얼굴은 칠하지 않는다. 모자챙과 선글라스 사이, 선글라스와 옷깃 사이는 배경이
 * 그대로 드러난 자리다. 원본이 그렇게 생겼고, 그래서 "얼굴을 감춘 사람"으로 읽힌다.
 *
 * ⚠️ 잉크를 20.5/24 로 맞춘 건 옆의 Material Symbols 아이콘과 크기를 맞추려는 것이다.
 * 24 를 꽉 채우면 이 항목만 커 보인다. 자유의여신처럼 BLEED 로 되당길 필요는 없다.
 */
function SpyIcon({ size = 20 }: { size?: number }) {
  return (
    <svg
      width={size}
      height={size}
    ...
    >
      {/* 모자 */}
      <path d="M 10.25 9.54 C 9.87 9.53 9.36 9.5 9.12 9.49 C 8.88 9.47 8.52 9.45 8.33 9.44 C 5.71 9.27 3.62 8.79 3.37 8.31 C 3.1 7.8 4.29 7.41 7.37 7 C 7.94 6.93 7.96 6.92 8 6.58 C 8.01 6.48 8.04 6.32 8.05 6.22 C 8.07 6.13 8.11 5.86 8.14 5.63 C 8.33 4.36 8.56 3.05 8.64 2.84 C 8.69 2.7 8.95 2.45 9.16 2.36 C 9.46 2.21 9.85 2.27 10.62 2.57 C 11.59 2.95 11.8 2.93 13.01 2.38 C 14.37 1.75 14.91 1.87 15.24 2.9 C 15.4 3.39 15.9 5.53 16.14 6.68 C 16.19 6.95 16.22 6.98 16.57 7.03 C 16.78 7.06 17 7.09 17.44 7.15 C 17.81 7.21 18.42 7.31 18.66 7.37 C 18.76 7.39 18.91 7.42 19 7.44 C 19.71 7.59 20.31 7.86 20.4 8.06 C 20.84 8.99 16.02 9.69 10.25 9.54 Z" />
      {/* 선글라스 */}
      <path d="M 13.6 12.34 C 13.01 12.22 12.6 11.92 12.33 11.43 C 12.05 10.9 11.7 10.89 11.45 11.41 C 11.24 11.85 10.62 12.28 10.12 12.34 C 9.27 12.44 8.8 12.26 8.33 11.69 C 7.8 11.02 7.83 10.58 8.43 10.38 C 8.79 10.26 10.23 10.29 10.75 10.43 C 11.31 10.59 12.1 10.6 12.91 10.46 C 14.3 10.21 15.21 10.23 15.63 10.51 C 16.18 10.89 15.47 12 14.46 12.35 C 14.28 12.41 13.9 12.41 13.6 12.34 Z" />
      {/* 어깨와 옷깃 */}
      <path d="M 2.21 22.22 C 2.06 22.13 2.28 20.61 2.52 20.02 C 3.1 18.65 4.48 17.69 6.24 17.45 C 6.68 17.38 6.76 17.36 6.76 17.3 C 6.76 17.25 6.49 16.85 6.3 16.6 C 5.6 15.69 5.22 15.02 5.22 14.69 C 5.22 14.25 5.97 13.9 7.68 13.53 C 8.52 13.35 8.5 13.35 8.56 13.58 C 8.74 14.19 9.53 15.19 10.19 15.64 C 11.47 16.5 12.1 16.59 13.14 16.05 C 14.33 15.43 15.12 14.59 15.44 13.58 C 15.51 13.37 15.6 13.37 16.51 13.57 C 18.2 13.95 19 14.37 18.89 14.82 C 18.82 15.09 18.16 16.12 17.56 16.88 C 17.31 17.2 17.28 17.27 17.35 17.34 C 17.39 17.39 17.42 17.39 17.9 17.46 C 19.78 17.72 21.25 18.9 21.7 20.49 C 21.83 20.97 21.94 22.08 21.87 22.19 L 21.83 22.25 L 12.04 22.25 C 5.44 22.25 2.24 22.24 2.21 22.22 Z" />
    </svg>
  );
}
...
const COMING_SOON: { label: string; badge: string; tip: string; after: string; icon?: string; Glyph?: Glyph }[] = [
  ...
  { label: "내부자 리포트", badge: "준비 중", tip: "현재 열심히 개발 중입니다!", after: "/kadera", Glyph: SpyIcon },
];
