...
const COMING_SOON: { label: string; badge: string; tip: string; after: string; icon?: string; Glyph?: Glyph }[] = [
  ...
  //
  // 아이콘 `preview` 는 창 안에 눈이 든 그림이다. 이름 그대로 '미리보기' 를 가리킨다.
  ...
  // ⚠️ 바로 위 내부자 리포트의 `contact_page` 와 **둘 다 네모 안에 뭐가 든 모양**이다.
  // 20px 에서 사이드바에서 제일 닮은 한 쌍이 됐다(세로로 두 칸 떨어져 있어 지금은 견딘다).
  // 둘 중 하나를 바꿀 일이 생기면 이 쌍부터 볼 것.
  {
    label: "국장 미리보기",
    badge: "준비 중",
    tip: "현재 열심히 개발 중입니다!",
