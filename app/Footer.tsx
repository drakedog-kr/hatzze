...
const SOURCE_GROUPS: { label: string; items: string }[] = [
  ...
  /* 배당으로 살기가 여기서 나온다. 화면 안에는 출처 이름이 없으므로(물음표 툴팁에서도 뺐다, 2026-09-13)
     ...
     국내 ETF 분배금(SEIBro 분배금지급현황, 2026-09-16부터)도 한국예탁결제원이라 위 이름에 든다. 그전의 미래에셋 TIGER 사이트는
     적지 않았다(2026-09-13 결정 — 출처 표기를 조건으로 건 약관이 없었다).
...
export default function Footer() {
  ...
  return (
    ...
    <footer style={{ marginTop: 64, borderTop: `1px solid var(--c-divider-strong)`, paddingTop: 40 }}>
      <div style={{ display: "flex", flexWrap: "wrap", gap: "40px 72px", justifyContent: "space-between" }}>
        {/* 브랜드 + 키워드 소개글 */}
