 ...
 * 개인이 미국 주식을 사는 주된 길은 국내 증권사를 거친 직접 매매이고, 이 층은 그 옆길이다.
 *
 * ⚠️ 예전엔 여기에 "보유의 62.9% 가 보통주, 20.6% 가 펀드·ETF"라고 적혀 있었다.
 * 미 재무부 SHL 조사값인데 그건 **국민연금까지 포함한 전 국민** 숫자라 이 페이지가
 * 말하는 개인과 모집단이 다르다. 그 카드를 뺄 때 이 인용도 같이 뗐다 — 개인의 종류별
 * 구성을 주는 원천은 아직 없다.
 *
